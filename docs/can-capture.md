...
`bringUpCan()` took `can0` **down** before bringing it up, on every service start. `ip link set can0 down` ends the `candump` behind `can-capture.service`, whose unit carries `Restart=on-failure` / `RestartSec=5`, so systemd waited five seconds and `capture.sh` opened a **new file**.
...
1. **The kernel refuses to reconfigure a live CAN device.** `drivers/net/can/dev/netlink.c` returns `-EBUSY` for bitrate (`:462`, `:611`), ctrlmode (`:315`) and `restart_ms` (`:659`) while `IFF_UP` is set. So taking the interface down is the _only_ way to set any of them.
...
Anything else takes the original path, unchanged. The journal line names every field it read, on both branches, so one line explains the whole verdict.
...
⚠️ **The `BUS-OFF` branch is near-unreachable, and is not a safety net.** `can_bus_off()` (`drivers/net/can/dev/dev.c:264-278`) schedules an automatic restart after `restart_ms`, and the Korlan's `usb_8dev` driver provides `do_set_mode` (`usb_8dev.c:932`), so with `restart-ms 100` the kernel is already resetting the controller within 100 ms and `ip` will rarely catch the device in that state at all. It refuses the skip because refusing is conservative, not because it rescues anything.
...
3. **Whether `ctrlmode_supported` is populated.** `usb_8dev.c:934-935` sets it including `LISTEN-ONLY`, so it probably is — and when present it is logged, because it is positive proof that this kernel and `ip` render ctrlmode arrays at all, corroborating the inference above rather than resting on it.

The expected line is:

```
can: can0 is already up @500k ACTIVE (TX enabled) — skipping the down/up (state=ERROR-ACTIVE bitrate=500000 restart_ms=100 ctrlmode=[] ctrlmode_supported=[…])
```
