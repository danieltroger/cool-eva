...
Worth recording the other direction too, because it is not written down anywhere else: **ten scratch scripts** (`headlight-off`, `lpr-mode`, `dash-command-sweep`, `reboot-vcu`, `dash-command-write`, `beam-threshold`, `probe-charge-stop`, …) call `bringUpCan("can0", true)`, and every one of them now skips on a bus the service already left ACTIVE. The garage-scripting path gets the same benefit as the deploy path, for free.
