...
- **`stand_up`** — 0 for the entire AC session (**all 317 785** frames) and for the whole bounded DC one (**all 106 100**), and it is tempting, because a deployed side stand is a _physical_ interlock the Energica will not enter Go against. Rejected **for the hazard of the fan running while the bike moves**, which `go` covers completely and in milliseconds. ⚠️ That is not the only hazard this mode creates, and the other one is the reason `stand_up` stays on the table: the mode trains a rider to sit twisting the throttle of a parked motorcycle for minutes at a time, and `go` is a **lagging** indicator of that — it reads 1 only once the bike already can move, by which time the wrist is on the grip. `stand_up` is the only signal on this bus that speaks to whether the bike can be _put into_ that state while a session runs. Two independent charges say it costs nothing in availability, unlike `key_on` (**38.2 %** AC / 70.2 % DC, both bounded). 🚨 **"`stand_up` has never been observed as 1 anywhere in this archive" stood here until 2026-09-14 and is FALSE.** b1 bit 5 is set in **3 964 801 of 15 006 856** frames (26.4 %), with 31 rising edges across 24 of the 97 files carrying 0x102, and `rides.db` has **147 of 442** rows reading 1. It is true only of the two charge sessions the tables above measure, where the bike was on its stand throughout — the scope was lost between a session-level measurement and an archive-level sentence, and `docs/can-decode-findings.md` §"Byte 1" has recorded the opposite ("the garage lap caught `stand_up` toggling with the rider's actions") since August. ✅ The bit works, and its polarity is corroborated: **99.66 % of the frames where it is set also have `go` set**, which is what an interlock that must be satisfied before the bike will move looks like.

**What actually stops it being added today** is the second reason, which this section already gave and which the falsified one was obscuring: the mode's hazard is a rider twisting the throttle of a parked motorcycle, `go` covers the movement hazard completely and in milliseconds, and `stand_up` would buy only the _leading_ half of the other one. That is a trade-off to decide, not a measurement to take. ⚠️ Deliberately **not** decided by the change that corrected this text (#217/#218 decoded 0x101 and the rest of 0x102 and touched no gate); the availability figures above say it costs nothing, and the call is the owner's.
...
- **`stand_up` is decoded but unused here.** It reads 0 through both charges, which is consistent with the decode: the bike was on its stand for both. 🚨 This bullet used to add "the bit has never been seen as 1 anywhere in this archive, so 'the stand is down' and 'the bit is stuck' are indistinguishable" — **false, corrected 2026-09-14.** Archive-wide it is set in 3 964 801 frames (26.4 %) with 31 rising edges, and in 147 of 442 `rides.db` rows. There is nothing left to measure; §4's rejected-alternatives list says what it would buy and what it would cost, and that is now the whole of the question.

### The handlebar gesture: a hold of MODE ENTER steps the fan

> _"can we have a physical-bike-button-shortcut to toggle fan mode between manual 100 % and automatic"_
...
#### Why _off_ exists at all, and why it is the third state
...
⚠️ **Only the _off_ this gesture commanded is taken back.** A rider who stops the fan with the slider has said something that has always survived until the bike is switched off, and riding away must not quietly overrule it.

It is a beat rather than a change subscription for a reason that only shows up after a failure: `speed_can_kmh` carries a 0.5 km/h deadband, so a bike settled at a steady speed raises no change events, and an event-driven revert would be **one-shot** — if the `setMode()` were refused, nothing would ever try again.

#### What the rider hears, and the two cases where they hear nothing

The fan is the feedback: from rest the 1.5 s kick-start at 100 % is unmistakable.

⚠️ **Two documented cases are inaudible, and are not worked around.** Manual 100 % → automatic commands the same duty, and therefore changes nothing you can hear, when the pack is at or above the top of the curve, and for the whole of any DC session since #147. The dashboard banner (`public/lib/announce.js`) covers it for a rider who is looking at the phone, which is not the rider this was built for. Adding feedback hardware or a bus write to fix it is out of scope: nothing here transmits to the bike.
...
| ------------------ | ---- | ----------- | --------- | -------- | ---------------------------------------- |
| 09-03 18:30:37.066 | 33   | 0           | 37        | 41 ms    | **Fan: off**                             |
...
`commandManual()` is the only path that changes the mode and the duty in one action, so it is the only one that has to care: it publishes the mode **after** the awaited command. `switchMode()` was left alone by #203 — `/fan?mode=manual` commands no duty of its own — and the automatic and fun paths need no target to word their sentence at all. ⚠️ **That exemption held only while the controller was idle**, which is #206: a Manual tap landing _inside_ a curve command — during `beginKickStart`'s three awaited writes, say — was worded from a `fan_target_pct` the curve had not published yet, and said "Fan: off" over a fan being commanded 68 %, then corrected itself with a second banner. Symmetrically, a tap landing inside a stop said "Fan: manual 68 %" over a fan being stopped.

**The remedy is to wait, not to re-order, and that is forced rather than chosen.** `switchMode("manual")` has no command of its own to publish behind, so the two candidates were: publish the controller's _current_ target alongside the mode, or wait for the controller's queue. The first cannot be made correct. `commandDuty()` sets `context.targetPercent` **before** its awaits on the running path, but `goIdle()` sets it to 0 **after** all three of its writes — and that ordering is the enables-before-output safety property this file exists for, so it is not available to be moved. Mid-stop, "the controller's current target" is still the running duty, and publishing it would announce "Fan: manual 68 %" over a fan being stopped: the second symptom, not a fix for it.

So `FanController` gained `settled()` — a no-op queued at the tail of the same `runExclusively` chain, which inherits its ordering rather than reaching into the queue's internals — and the manual branch awaits it before publishing. ⚠️ **What it waits for is work IN FLIGHT**, a handful of sysfs writes: the kick-start's 1500 ms tail is queued later, from a timer, and is not in the chain at tap time. `context.mode` is still assigned _before_ the await, so the curve (which reads it in `evaluate()`) cannot command during the wait, and `publishDecision(null)` is guarded on the mode exactly as `commandManual()`'s is — a "back to Auto" tap landing inside the wait has already published the curve's real decision. `scripts/check-fan-banner.ts` §9a and §9b hold both directions against a bridge parked mid-command, and §9c holds the guard.

⚠️ **Two costs, both small and both written down rather than discovered.** The tap's own reply string is computed after the await and is _not_ guarded the way the publish is, so a `/fan?mode=automatic` landing inside the wait leaves the manual tap answering "Manual. The fan holds 68 %…" while the wire correctly says automatic — last writer wins, and the wire is what the phone reads. And on a bridge that never settles, `/fan?mode=manual` now hangs where it used to answer: that is Manual joining Automatic rather than a new class of failure — `setMode("automatic")` has always awaited `runTick` → `setDutyPercent` — but it does mean no fan control answers over a wedged bridge. A timeout would be the wrong fix; it reintroduces the race this closes.
