...
`commandManual()` is the only path that changes the mode and the duty in one action, so it is the only one that has to care: it publishes the mode **after** the awaited command. `switchMode()` is left alone here — `/fan?mode=manual` commands no duty of its own — and the automatic and fun paths need no target to word their sentence at all. ⚠️ **That exemption holds only while the controller is idle**, and the review of #203 measured the hole: a Manual tap landing _inside_ a curve command — during `beginKickStart`'s three awaited writes, say — is worded from a `fan_target_pct` the curve has not published yet, and says "Fan: off" over a fan being commanded 68 %, then corrects itself with a second banner. Same defect, different path, and #206 carries it; it is not fixed here because the remedy is to wait on the controller's queue rather than to reorder two records.
