...
**Re-decided rather than blanked, and after the clears.** The enum has no "unknown" arm, so blanking would mean picking a constant and calling it a reading; a re-decide gives a sentence that is true at the instant it is recorded. It runs after the five clears so the verdict sees the emptied rings and the cleared stand-down rather than the session that just ended — `setMode` has done exactly this since #193, and like `setMode` it puts nothing on the bus: `decide()` only reads.

⚠️ **What it records is what the controller WOULD decide, not what it did.** Leaving a session is unambiguous — no DC session, so `NOT_DC`, "Waiting for a DC fast charge." Entering one answers `NO_HISTORY` only when the mode is automatic, `batt_temp_hi` is present, plausible and fresh, the ceiling has arrived, **and the pack is under 54 °C**; a pack arriving at a rapid charger at 54 after a hot run — the case this controller exists for — answers `BLIND_DESCENT` instead, which is a `kind: "command"` verdict whose _reason_ is recorded while nothing is commanded. `scripts/check-charge-auto.ts` §19 pins its fixture below the cliff for exactly that reason.
