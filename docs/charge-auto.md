...
⚠️ **That rests on a precondition, and it is a property of the RING rather than of the estimator: every sample is a crossing instant.** The one reading that is not is a process's first-ever, which `record()` delivers as a change because there is nothing to compare it to; the one that sits a point high is a downward crossing. Neither reaches the ring — the CAN channel records a `soc` 530-1282 ms before the controller subscribes in all 77 lives that started it, and 304 of 304 consecutive `soc` pairs inside a DC session are `+1` — but nothing _enforces_ either, so `rememberSoc` says so in the journal when it does keep such a sample and `src/index.ts` carries the ordering at the call site. Both measurements, both failure modes and the two unguarded paths that are **not** bounded by one count: `docs/dc-taper.md` § "The first SOC sample, and why it is a crossing".

⚠️ **And the assertion behind it is a property over trajectories, not three instances.** `check-charge-auto.ts` §17 computes its `truth` from the same two endpoints the estimator reads, which catches arithmetic drift and cannot see the assumption underneath. `scripts/check-soc-rate.ts` builds the ring from a **continuous** trajectory the way the bus builds one — sample at 20 Hz, quantise to a whole percent, emit on change — so `truth` is the trajectory's own mean rate over the span the estimator claims. Two mutations that the rest of the suite passes go red there, measured by exit code: the window filter losing its `<= nowMs` clause (no existing fixture holds a sample from after its own `nowMs`), and the `advanced <= 0` guard deleted (§17's backwards probe passes it at −0.2 %/min, because both of its tests are upper bounds — and a negative rate comes back through `minutesUntilTaperBites` as a **negative horizon**, which makes the predicted headroom larger and fires the veto on a pack that is not charging).
...
⚠️ **The third edge is the session boundary, and it used to pass unmarked.** Until #204, `forgetSession()` cleared four fields on a `charge_manager_state` edge and recorded nothing — `context.reason` included — so for up to one 60 s tick `/charge-auto` answered with the _previous_ session's sentence over `commandedAmps: null`, reachable on any re-plug at the same charger quick enough that no tick intervenes. It now **re-decides and records the reason there**, so the endpoint's answer is true from the edge, and the record is itself a patch whenever the reason moves.

**Re-decided rather than blanked, and after the clears.** The enum has no "unknown" arm, so blanking would mean picking a constant and calling it a reading; a re-decide gives a sentence that is true at the instant it is recorded. It runs after the five clears so the verdict sees the emptied rings and the cleared stand-down rather than the session that just ended — `setMode` has done exactly this since #176, and like `setMode` it puts nothing on the bus: `decide()` only reads.

⚠️ **What it records is what the controller WOULD decide, not what it did.** Leaving a session is unambiguous — no DC session, so `NOT_DC`, "Waiting for a DC fast charge." Entering one answers `NO_HISTORY` only when the mode is automatic, `batt_temp_hi` is present, plausible and fresh, the ceiling has arrived, **and the pack is under 54 °C**; a pack arriving at a rapid charger at 54 after a hot run — the case this controller exists for — answers `BLIND_DESCENT` instead, which is a `kind: "command"` verdict whose _reason_ is recorded while nothing is commanded. `scripts/check-charge-auto.ts` §19 pins its fixture below the setpoint for exactly that reason — and below the cliff at 55, where `HARD_CEILING` answers before this arm is reached at all.

⚠️ **A side effect at boot, worth naming so nobody diagnoses it as a bug.** `context.lastSessionState` starts `null`, so the first `charge_manager_state` the controller ever sees is an edge. Since #204 that means a reason is recorded within a frame of subscribing instead of up to 60 s later — an improvement, and new behaviour.

**And the browser still clears its own flag.** `onChargeSessionEnd()` clears the tile's `loaded`, alongside the two sibling charge controls. That is not made redundant by the Pi's fix and the two do different jobs: the Pi's makes the _endpoint's answer_ true, while `loaded` is what stops the _tile_ painting the previous session's sentence during the round trip that fetches the new one — on the next plug-in the write gate's reply wakes the render synchronously and the re-read asynchronously. Deleting it has been proposed twice; `scripts/check-charge-auto-live.ts` §7 now parks that reply and goes red if it is deleted a third time.

⚠️ **A Pi restart mid-charge — a deploy — is the same shape, and mostly falls out.** `startChargeAutomatic` records `charge_auto_mode` at start-up (`publishMode` at `auto.ts:136`) and **not** `charge_auto_reason`, which waits for the first tick a minute later; `public/lib/store.js` never removes a key, so the browser keeps the pre-restart reason and amps and no guard moves. What saves it is the link, not the signals: the socket goes quiet, `public/lib/connection.js`'s `SILENCE_LIMIT_MS` (12 s) declares it dead, every reading staleness-gates, `sessionLive` drops and the session-end path above runs — so the reconnect arrives as a fresh session and re-reads the Pi. A restart that completed inside 12 s would slip through and leave the tile claiming a current the new controller has forgotten, for up to one tick. Not observed; a Node 24 service start on a Pi Zero is not usually that quick, and it is written down rather than guarded because the remedy would be a browser-side guess about the Pi's identity.
...
⚠️ **The two reads of one tick can be answered out of order, and that reproduced #200 through #200's own fix.** 60 s is the spacing between _ticks_, not between the two reads one tick issues — those are tens of milliseconds apart, and the first carries `commandedAmps` from before the command. Replied out of order, which one retransmit or one wifi power-save wake-up is enough to cause, the stale reply lands last, and because a settled controller then records nothing further the tile holds the pre-command number **for the rest of the charge**. Measured to need about 45 ms of skew between the two round trips. This was **not** a pre-existing race: watching the reason alone, one tick fired one read. So the wake-up on the commanded current ships with a read counter — `refresh()` drops an answer that a newer read has already spoken over, and a POST takes a number rather than checking one, because its reply is the only one carrying the refusal `message`. `scripts/check-charge-auto-live.ts` §8 lands the two replies backwards and asserts the tile ends on the commanded current.

⚠️ **A read that fails must not count as a read that acted.** The guards are advanced before the round trip, so a `fetch` that throws would otherwise leave them agreeing with the signals and nothing would ever re-deliver the change — trap #2 in `docs/dashboard-decisions.md`, on the failing path rather than the gated one. Measured before the fix: one dropped read, then a settled controller, left the tile on the wrong reason _and_ the wrong current through 40 heartbeats with zero retries. `refresh()`'s `catch` now rolls both guards back, so the next heartbeat tries again; §9 asserts both that it recovers and that it does not become a poll.
...
⚠️ **What the session-ahead veto costs when it is wrong.** `TAPER_GRID` in `scripts/charge-auto-plant.ts` runs 60 → 100 % SOC with **no taper at all**, so the veto suppresses steps on the strength of a taper that never arrives — the harshest arrangement there is. Over that grid it fires on **152 of 2 984 ticks**, adds **no crossing of 55 °C**, costs at worst **1.6 min**, saves at best 2.3, and reverses 4 times against the shipped rule's 5. It fires **zero** times on the replays §3, §4 and §6 pin their numbers over, which is why those numbers are unchanged — asserted rather than assumed, because otherwise they would stop meaning what their comments say and nothing would notice. ⚠️ **§11's frozen grid is the exception, and an earlier version of this sentence had it wrong by lumping the two together**: the veto fires **33 times on 15 of those 150 plants**, and §11's golden 16 crossings survive despite that rather than because the veto never arrives. Both counts are pinned (`EXPECTED_FROZEN_GRID_VETOES`), because "16 crossings" means something different if the veto stops reaching that grid.
