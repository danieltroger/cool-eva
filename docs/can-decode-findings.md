...
⚠️ **The sweep is about FLAGS, and two numeric siblings are known and deliberately untouched.** The same payloads run through an arithmetic sweep name exactly two more: `pack_kw` == `pack_v` × `pack_a` / 1000 on 0x200 (`decode-bms.ts`) and `cell_spread_mv` == `cell_max_mv` − `cell_min_mv` on 0x203, both computed by our decoder from two keys the same frame already logs. `cell_spread_mv` is the exact analogue of `brake` — three keys, no deadband on any of them, so the spread is recomputable row for row. `pack_kw` is not quite: it carries a 0.05 kW deadband its parts do not, so its rows are a filtered series rather than the product of the stored ones, and removing it would change what the log holds rather than just where the arithmetic happens. Neither was touched here; they are recorded so the next person does not have to re-derive them, and the Grafana panels that read them are the work either would cost. ⚠️ Two more that look like this and are NOT: `speed_can_kmh` is exactly `motor_rpm_can` / 42.0 and `kwh_per_100km_can` is 100 / `km_per_kwh_can`, but each pair arrives as two separate fields on the wire — that arithmetic is the VCU's, and what this log stores is what the bus said. Off-CAN, `motor_power_kw` (BLE) is torque × 2π × rpm, but the rpm in that frame is not logged anywhere, so the power is the only record of it.
