...
⚠️ **This paragraph used to say the charge manager's own `CM_ERROR_SOURCE` / `CM_ERROR_CODE` "is not broadcast anywhere and needs a diagnostic session". That was wrong.** It is on `0x610` b1 and b2-3 at 10 Hz, decoded since 2026-08-20 once Energica's factory DBC named the bytes — see `docs/charge-manager.md` §"b1-3 are a fault code". The two are worth keeping side by side rather than one replacing the other: ⚠️ this said "through all three fault episodes `0x610` carried a source and a code while **this bit stayed 0**", and it is wrong in both directions: the bit rises 0.162 s AFTER a code in three episodes, and in a fourth it holds 269.7 s with no code at all. So the rollup is not a summary of them.
...
⚠️ **Byte 7 bit 1, `ERR_ChargeCM_Out`, is 0 in all 105 736 of them — and that sample was too small.** Archive-wide the bit is set in **3 563 frames across four windows**; in three of them it rises exactly 0.162 s after `0x610` publishes a charge-manager error code, and in the fourth it holds 269.7 s with `0x610` b1-3 at zero. The 105 736 frames were the ones scanned, and none of them contained a fault — including every frame of the 2026-08-04 DC session, which ran to 20.04 kW and 66.2 A and completed cleanly. So the bit is well-formed, and "never seen to fire" was a statement about the sample rather than about the bike. See `docs/charge-manager.md` § "The fault corpus". ⚠️ **Both sentences that used to close this paragraph were wrong**, and they are the reason the claim survived so long: "no capture on this disk contains a charge-manager fault" and "this decode has never been seen to fire". Four faults are on this disk, in three capture files, and the bit is set in 3 563 frames. The 2026-08-09 12:38:35-12:42:35 window is not the only time the charge manager has complained and no longer needs recovering. See `docs/charge-manager.md` § "The fault corpus".

🔎 **For whoever recovers those 2026-08-09 captures — the question has changed.** This paragraph used to call them "the check that would confirm it outright", and that is stale: the bit position is settled by 3 563 frames across four windows, three of which lead an independent ECU's error bytes by 0.162 s on three separate days. Three independent events beat four edges in one window. What those captures could still add is a **fifth episode with its own `(src, code)` pair** — the corpus has exactly two code values, 853 and 1101, and a third would be the difference between a list and a pattern. The bit should still go **1 → 0 → 1 → 0 at 12:38:35 / 12:39:36 / 12:41:36 / 12:42:35**, byte 7 stepping 0x03 → 0x01 → 0x03 → 0x01; read `0x610` b1-b3 across the same window and see whether it carries a code, or is a fourth-event-style fault with none.
...
`src/can/decode.ts` (byte 0 bit 6, bytes 1-2), `src/can/vcu-digitals.ts` (byte 0's other seven bits, byte 3) and `src/can/attitude.ts` (bytes 4-7). 100 Hz.
...
⚠️ **The sweep is about FLAGS, and two numeric siblings are known and deliberately untouched.** The same payloads run through an arithmetic sweep name exactly two more: `pack_kw` == `pack_v` × `pack_a` / 1000 on 0x200 (`decode-bms.ts`) and `cell_spread_mv` == `cell_max_mv` − `cell_min_mv` on 0x203, both computed by our decoder from two keys the same frame already logs. `cell_spread_mv` is the closest analogue of `brake` — three keys, no deadband on any of them, so the spread is recomputable row for row — but it is **not an exact one**, and the difference is the reason to leave it alone. `cell_deviation_mv` (0x205, the BMS's own max−min, mem 2063) and `cell_spread_mv` are an independent cross-check on each other: `src/can/decode-bms.ts` records that they agreed exactly, at 9 mV, on the 2026-08-02 capture, so either one disagreeing later means a misread frame. Deleting the spread costs that check, and `brake` had nothing of the kind to lose. `pack_kw` is not quite either: it carries a 0.05 kW deadband its parts do not, so its rows are a filtered series rather than the product of the stored ones, and removing it would change what the log holds rather than just where the arithmetic happens. Neither was touched here; they are recorded so the next person does not have to re-derive them, and the Grafana panels that read them are the work either would cost. ⚠️ Two more that look like this and are NOT: `speed_can_kmh` is exactly `motor_rpm_can` / 42.0 and `kwh_per_100km_can` is 100 / `km_per_kwh_can`, but each pair arrives as two separate fields on the wire — that arithmetic is the VCU's, and what this log stores is what the bus said. Off-CAN, `motor_power_kw` (BLE) is torque × 2π × rpm, but the rpm in that frame is not logged anywhere, so the power is the only record of it.
...
⚠️ **The carry-forward has to start before the window, and getting that wrong is worse than the problem it solves.** The first version of the lane carried each half forward from its first row **inside** `$__from..$__to` and read a half with no row yet in the window as released. Both halves are log-on-change, so a lever held from before `$__from` has its `1` outside the window and no row inside it — and the lane then drew `no` over a brake that was being squeezed. That is a class of error the retired query could not make: every point it drew came from a `brake` row and carried that row's value, so it could omit points but never invent one. The fix is to read each half up to `$__to`, keep its last row at or before `$__from`, and clamp that row onto the left edge so it does not widen the panel. `src/can/signals.ts` logs the first sample of every signal after each boot, so a window that opens before a ride was never affected; what was affected is a window whose left edge lands inside an application, measured at ~0.75 % of bike-on time for the front circuit.

✅ **Verified against the archive rather than argued.** Every 0x102 frame on disk — 14 638 105 of them, all 246 captures plus `ride-1.log` and `buttons-2026-08-19.log` — was decoded and passed through the log-on-change rule `src/can/signals.ts` applies, giving 1 000 `brake` transitions, 974 `front_brake` and 43 `rear_brake`. Loading those into a database with the real schema and a cutover placed in the middle of the corpus (502 legacy `brake` rows before it, 512 half rows after) the rewritten lane reproduces the old one **segment for segment, 1 000 of 1 000**. The same corpus through the naive `MAX()` version collapses to a single `yes` segment.

⚠️ **The captures were replayed IN FILE SEQUENCE, not merged into one time-sorted stream.** This paragraph said "sorted by time" until 2026-08-30 and it was wrong, by a factor of about 45. The archive is 246 separate `candump` sessions, **85 of the 92 files carrying 0x102 overlap another file in time**, and several record the same minutes from different runs — so a global sort interleaves them and a bit that is 1 in one capture and 0 in another produces a transition on every alternation. Re-measured over the whole corpus: **front 42 159, rear 2 709, `brake` 44 853**. Log-on-change is a rule about ONE stream in the order it arrived, so the only reading of it that means anything is one file after another with each file's own frames in order — and that is what produced the numbers above, which also square with the 491 front applications and 18 rear counted independently further up. Anyone re-deriving these from a sorted merge gets tens of thousands and concludes something is broken.

⚠️ **Expect the last few transitions to move.** An independent replay in filename order gives front **977**, rear 43, `brake` **1 003** rather than 974 / 43 / 1 000, and the sorted merge has come out at 44 853 in one run and 53 133 in another. Whichever order a replay picks, transitions land on the file boundaries: which capture follows which decides whether a boundary is a change, and nothing about these files fixes that order. Treat the three-transition spread on the sequential figures as the resolution of this measurement, and the sorted figure as not a measurement at all.

✅ **And against the live database, which is what the archive replay cannot reach.** The owner's 269 MB `rides.db` (6 200 564 readings) predates 2026-08-19 entirely: 995 `brake` rows, no `front_brake` or `rear_brake` rows at all. Over its full time range the rewritten lane returns **995 rows and 917 merged segments, identical row for row** to the query it replaces, at 0.32 ms against 0.28 ms (median of 20, warm page cache; the first run of either is a few milliseconds and measures the warm-up rather than the query — that is where a reported 3.4 ms came from). `EXPLAIN QUERY PLAN` shows every arm index-driven: the cutover `MIN()` and the pre-window seed both resolve through `SEARCH … USING COVERING INDEX idx_reading_sig_ts`, and nothing scans `reading`. That is the legacy arm and the `COALESCE(MIN(...), 9223372036854775807)` sentinel proved on real data — a database where no half has ever been written is exactly the case that would otherwise blank the panel.

`scripts/check-brake-lane.ts` holds all of it against a database built by the real writer on every run; the archive itself is not in the repo, so that check builds its series rather than replaying one. ⚠️ It runs the query over **three** windows, and the second is not decoration: the archive replay used a single window spanning the whole corpus, which by construction cannot exercise the window's own left edge, and that is why the held-brake error above survived a 1 000-of-1 000 match.
...
**It is the only decoded handlebar bit in the CAPTURE archive with no long press anywhere** — every other button that is held for anything reaches 1.2 s to 191 s. That is what qualified it to carry a 1200 ms hold gesture (`docs/handlebar-gestures.md`), which has the full per-button table. ⚠️ That claim is scoped to the 268 captures: the **ride log** holds four ENTER presses at or past 1200 ms, the longest 4 260 ms (measured 2026-09-10), all of them deliberate holds for the bike's own dash menu. They do not move the threshold — no hold length can separate a deliberate ENTER hold from a fan gesture — and they are listed in `docs/handlebar-gestures.md` §"Long ENTER presses in the ride log".
...
🚨 **This section's heading is kept because code comments cite it by name, but its claim was wrong and the full account now lives in [can-0x410.md](can-0x410.md).** The frames carry the Connectivity-Hub message set byte-for-byte — byte 0 the type, byte 1 the sub-index — but **the instrument cluster transmits them**; the hub reaches it over UART and is the Bluetooth transport, not the author. The cluster also _synthesises_ each record from its own variables rather than forwarding bytes, so the CAN and BLE copies of one message type can disagree. Confirmed in `obd-garage/captures/2026-08-02_bms_90s.log`, which carries `1A 00`/`1A 01`/`1A FE` (GPS), `02 00`…`02 FE` (vehicle status), `04 00`/`04 FE` (odometer) and `00 FF` (the handshake seed) on that id.
...
**0x400 joined on 2026-08-16 and is the one entry that costs something.** It is the highest-frame-rate id on this bus and it carries almost no information: **its payload changed six times in 1 099 357 frames.** Worse, the rate is padding — every one of the 77 355 sub-7 ms gaps measured in one capture repeated the previous frame's eight bytes EXACTLY, with no exceptions, and the frame count per second swings between 0.8× and 1.2× of 0x102's steady 100 Hz depending on what the dashboard is doing (80 Hz through the DC charge, ~120 Hz while riding). So this buys ~100 RX wakeups a second on a Pi Zero for a button byte, plus five `record()` calls per frame. It is worth it only because the buttons cannot be read any other way, and log-on-change means an unpressed button still writes exactly one row per boot.
