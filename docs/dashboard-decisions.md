...
⚠️ **The HOLD gestures left this page on 2026-09-08 and are now the Pi's** — a 1200 ms hold of MODE ENTER steps the cooling fan, a 1000 ms hold of `btn_indicator_cancel` saves a waypoint, both recognised in `src/gestures/`. They had to move: `lib/connection.js` closes the socket whenever the page is hidden, so a phone in a pocket recognised nothing, which is every gesture worth making. `docs/handlebar-gestures.md` has the recogniser, the thresholds and the corpus behind them, and it is where the `LONG_PRESS_MS` argument that used to be in this section now lives — corrected, because part of it had gone stale (see below).
...
`LONG_PRESS_MS` (1200 ms) and `IMPLAUSIBLE_HOLD_MS` (30 s) were here until 2026-09-08. The first is now two constants on the Pi — 1200 ms for the fan cycle on MODE ENTER, **1000 ms** for the waypoint on the cancel switch, because that switch turns the hazard lights on if it is held to about two seconds. The second is gone entirely: it guarded against `src/gps/clock.ts` stepping the server clock mid-press, and the Pi measures on a monotonic clock that cannot be stepped. Both arguments, in full: `docs/handlebar-gestures.md`.
...
The case that settles it landed the same day: holding `btn_indicator_cancel` saves a waypoint — since 2026-09-08 at 1000 ms and recognised on the Pi (`docs/handlebar-gestures.md`), which changes nothing about this tile. So a key whose name, prefix and 762 recorded presses all say "momentary" is deliberately held past a second as a designed input — and the tile says "held 1 s" while it happens, which is the useful thing to see while you are waiting for the toast. Any list of held-state keys written yesterday would have been wrong about it today.
...
**Drive is white at every load, and the CALM → WATCH → WARN → BAD ramp is gone rather than retuned.** Two reasons, the second being the real one. The dashed derate rule is drawn _on top of_ the fill, and over the old BAD red it sat at **1.72:1** — in the one state where reaching into unreachable power is the thing to see. White takes that to 3.9:1 and needs no second colour to be legible against. And the ramp answered a worse question than the thing beside it: its thresholds were absolute (3, 15, 40 kW), while the hatching says how close you are to what the pack will _actually_ give you, which moves with heat, cold and SOC and is the number a rider can act on. Two colour languages for load, one of them fixed and wrong most of the time, is worse than one. The check holds the rule/fill contrast to a 3:1 floor, computed rather than asserted as a hex, so retuning either colour has to keep the pair legible.
...
The extremes are unaffected, being extremes rather than time claims: the discharge ceiling has been seen from 0 to 134.7 kW and the regen ceiling from 0 to 38.5. Those 5-11% are why the line's drop-versus-pin question mattered at all, and hatching is what retired it: a ceiling past the end of its half simply takes nothing away, and draws nothing, with no case to decide.

**The derate is hatched, and that dissolves both end cases.** This drew a single dashed line at the ceiling first. A line answers "where is the limit" and leaves the rider to measure the gap themselves at 90 km/h; hatching answers "how much has gone" directly. It also removes two special cases that cost a review round each:

- **A ceiling wider than its half hatches nothing**, which is exactly true — the pack is not what is limiting you. The line had no honest option here. Dropping it made "the pack allows more than the bike can take" identical to "`0x202` has not arrived", which is 5–11% of moving time (below); pinning it to the end made that case identical to a ceiling sitting _at_ full scale.
- **A ceiling of zero hatches the entire half**, the loudest thing this bar can say. The line drew on the centre divider, where it could be mistaken for it.

**There is no zero divider either**, and there was one — a 1-unit slate rect at the middle. Nothing needs it: the bar always spans the whole tile, so zero is the middle of a shape the eye already has, and the fill grows _from_ there, so its own inner edge marks the same point whenever there is any power to speak of. What it did instead was get in the way. Once the hatching went in, it had to be drawn _after_ the dashes to survive a 0 A ceiling at all — and it then read as a slate block interrupting a run of dashes, one more thing to work out, in the state with the least to say.

**The marking is a thin dashed rule, and the first attempt at it was unreadable.** It began as full-height blocks in a lighter slate, alternating with the track. That fails in a way no amount of reasoning about it surfaced and one screenshot did immediately: **the gaps between the blocks are the track's own colour**, so a gap and the still-available stretch next to it are literally the same pixels. Asked what the dark chunk after the fill meant — headroom, or the space between two marks — the honest answer was that you cannot tell.

So the marking is no longer the same height as the thing it marks. `HATCH_HEIGHT` is 0.3, a rule down the middle of the derated stretch at under a third of the bar, in a single muted grey with the track left alone underneath it. Full height competes with the fill for "this is the bar"; a short rule cannot be mistaken for the stretch it sits in, and the ambiguity disappears with it. Three states, three appearances: plain dark is headroom, dashed is gone, coloured is what you are drawing.
...
**The two ceilings travel as one object**, `PowerLimitsKw`, from `powerLimitsKw()` through `splitBar()` to `derateSpans()`, and the two scales travel as another, `SplitBarScale`. Two same-typed parameters side by side would be a pair a caller can cross, and a crossed pair draws a screen that looks entirely deliberate — the BMS allowing 96 kW of regen and 38 kW of drive. The first version had exactly that shape, and a check covering both _ends_ of the path stayed green with the sides swapped at the call site between them. One object makes the outer hop unspellable; `check-power-bar.ts` §6 walks the remaining one end to end with deliberately asymmetric amps, and now asserts each half is measured against its own scale rather than a shared one.
...
**The light ramp costs separation to buy contrast, and the trade is measured.** Today's accents are 1.2–2.8:1 on white, so a light theme needs its own ink ladder rather than a background swap. On a `#f1f5f9` tile the Tailwind 800 step is the FIRST that clears 6:1 — the 700 step tops out at 5.91 — and darkening yellow and orange that far walks them towards the same brown:
