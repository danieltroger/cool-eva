...
⚠️ **And some are simply wrong**, marked as such in the list so the next person inherits the judgement rather than the surprise. `vehicle_state` and `vehicle_substate` are the BLE twins of `vehicle_state_can` / `vehicle_substate_can`, which #234 gated — so one transport's copy of a quantity is gated and the other's is not. `speed_can_kmh` is ungated while `speed_kmh`, the same quantity down a different path, is gated to `[0, 300]`. Those are two-line fixes for whoever next touches those frames; they are out of scope for the change that found them, because a bound is a claim about the bike and each one needs its own evidence.
