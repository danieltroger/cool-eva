...
`MIN_HATCH_WIDTH` drops a hatched stretch narrower than about one dash period: below that it is a smudge at the end of the bar rather than a pattern, and a rider cannot tell a 2% derate from a rendering artifact. The hatching is drawn _under_ the fill, so where you have actually reached into unavailable territory the fill wins — that is the more urgent of the two facts.

⚠️ The hatch is the one stroke in `lib/svg.js` deliberately **not** `vector-effect: non-scaling-stroke`. Its width is the bar's height and its dash period is a fraction of the bar's length; both are meant to stretch with the tile so the texture keeps its shape from a 380 px phone to a 1000 px laptop. Everything else stroked in that file is a hairline, where stretching is the bug.
