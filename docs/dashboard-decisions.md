...
The machine is asymmetric. Discharge reaches −117.3 kW in the archive against the Ribelle's 126 kW peak; regen reaches **40.9 kW** with a p99 of 22.6 kW. So on a symmetric meter the regen half could never fill more than a third of its travel on the best day the pack has ever had.
...
- **Measured volts, not nominal.** The ceiling really does fall as the pack sags under load, and that is exactly when a rider wants to see it; a limit computed against a nominal 350 V would be frozen for the reason it most needs to move.
- **They earn the space because they move.** Over 1054 minutes of moving time the discharge ceiling averages **91.3 kW** against the bike's 126 kW peak, and the regen ceiling 25.3 kW with a maximum of 39.2. Of the discharging `pack_kw` samples only ~0.4% ever exceed the ceiling drawn against them: it really is a wall the meter approaches and rarely crosses, which is what makes it worth drawing.
...
Unlike the one it replaces this is stable under its own knobs: across hold caps from 1 s to 5 min the mean moves 92.6 → 90.0 kW and the share 5.5% → 8.0%. An independent re-measurement during review landed at 94.3 kW and 10.9%. Every route agrees the share is somewhere between one minute in twenty and one in nine, and none is anywhere near 0.83%.
...
3. **A short dashed rule down the middle of the derated stretch**, which shipped. It reads at rest and the visor takes it: measured through the scatter-plus-veil simulation on a delivered render, those dashes average **1.48:1** against their track, and their brightest dash reaches 1.85:1.
...
**Zero is a gap in the track**, not a mark on it, so nothing is painted in a colour assumed for the ground. It is the only thing saying where zero is while the meter is empty; once there is power the fill grows from that point and marks it itself.
