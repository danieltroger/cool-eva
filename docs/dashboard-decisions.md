...
### The waypoint list — `views/waypoints.js`, `lib/waypoint-list.js`

The tile above it shows the LAST waypoint; this shows the boot. Three decisions in it are not obvious.

**It is drawn from `/status`, and the WebSocket is only a doorbell.** The obvious source is the `waypoint_*` signals the page already has — and it is wrong, for a reason that has nothing to do with this list: `lib/connection.js` closes the socket whenever the page is hidden (§"Do not hold a socket while the page is hidden"), nothing in `public/` takes a Screen Wake Lock, and `src/ws.ts` answers the next connection with a snapshot carrying the latest value of each signal and nothing before it. A phone that spent the ride locked in a pocket therefore knows about exactly ONE of the six waypoints saved on it — and the handlebar hold is used at riding speed (13 of 14 holds on 2026-09-07 were made at 30–119 km/h). `src/ws.ts` also drops patches to a client over `MAX_CLIENT_BACKLOG_BYTES` and does not bound a merely slow one, so a visible phone on garage wifi can miss them too. The Pi keeps the events instead (`src/gps/waypoint-log.ts`) and the list is exactly what it served, reversed.

**⚠️ The refresh folds on the VALUE, or it is a `/status` every five seconds.** `src/ws.ts` heartbeats a FULL snapshot every `HEARTBEAT_MS` and `lib/store.js` assigns each signal a freshly parsed object, while VanJS schedules on identity (`v !== this.rawVal`). So a derive watching `waypoint_seq` re-runs on every heartbeat, for ever, from the boot's first save — and `/status` walks the ride-log directory and `stat`s every file in it, on a Pi Zero that is also serving the socket and sealing the log. `foldAnnouncement()` in `lib/announce.js` is what it folds with, reused rather than copied: its `baselined` flag is already the answer to "the first arrival of an on-demand signal is not a baseline". ⚠️ Reading `valueOf()` instead of `.val` does **not** dodge the heartbeat — the dependency is registered by the `val` getter before the number is looked at.

**⚠️ …and it does NOT re-baseline on a dropped link, unlike the banner three files away.** `announceWaypoints()` blanks its memory whenever the link is not live, because a waypoint saved while the page was hidden is news from before we were listening and announcing it would be a lie about when. For a LIST that is backwards: a save made while the page was hidden is the whole reason the list exists, so the reconnect must be allowed to notice it and fetch it.

**Six rows, then `show all N`.** Fifty rows is ~2 000 px between the trip stats and "📍 Save waypoint here", on a sheet whose risk tiers are argued (§"the sheet's three risk tiers", `style.css`) on the premise that a thumb can reach them. The expander is `views/faults.js`'s `.code-toggle`, deliberately NOT `.code-toggle.risk-fold` — that variant is the irreversible-write fold, and borrowing a safety affordance for a list teaches it to mean nothing. `openSheet()` collapses it again, the same rule that disarms the two Pi actions: a section this sheet expanded must not still be expanded next time it opens.

**What can make the list short, and why each case has its own sentence**: `docs/waypoints.md` §"What the list can and cannot be short of". The counts in the payload are never capped, which is what makes those sentences possible.
