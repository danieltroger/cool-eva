...
A Pi that has never taken a reading shows one instruction, and after [#177](https://github.com/danieltroger/cool-eva/issues/177) it was still `node --experimental-strip-types scripts/read-freeze-frame.ts --lifetime --save, with the service stopped` — a shell command, on a phone, held one-handed at the bike. #177 had already made the read a button in the service sheet; the sentence just never moved. (It stood that way from #177 at 19:41 on 2026-09-08 to #187 the following evening — a day, not the month an earlier draft of this paragraph claimed. The duration was never measured and did not need to be: _the sentence never moved_ carries the argument on its own.) It now leads with the in-app path and keeps the command as a footnote for the case it is still the answer to, a Pi whose service is stopped.
