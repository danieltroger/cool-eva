...
A Pi that has never taken a reading shows one instruction, and for a month after [#177](https://github.com/danieltroger/cool-eva/issues/177) it was `node --experimental-strip-types scripts/read-freeze-frame.ts --lifetime --save, with the service stopped` — a shell command, on a phone, held one-handed at the bike. #177 had already made the read a button in the service sheet; the sentence just never moved. It now leads with the in-app path and keeps the command as a footnote for the case it is still the answer to, a Pi whose service is stopped.
...
⚠️ **The dot is a zero-length round-capped `<path>` with `vector-effect: non-scaling-stroke`, not a `<circle>`.** The viewBox is stretched horizontally (below), so a circle arrives as an ellipse — measured 9.8 × 6.1 px at 390 px. `sparkline()` uses the same property against the same stretch, and it makes this a true round dot at any width.
