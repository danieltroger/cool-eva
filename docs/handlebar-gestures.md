...
4. the gesture fires when a fresh sample shows the threshold passed — normally with the thumb still down, on the first beat at or after it.
...
⚠️ **98 of the 268 carry `0x102`, and only 97 of those are candump `-tA` text.** The 98th is `can_log-2026-06-14.txt`, 453 MB in `<epoch> <id> <hex>` form (its first `0x102` line is `1781428137.333 102 0002004499ffddff` — no `can0`, no `[8]`, and no spaces between the data bytes), which a `can0`-anchored `grep` silently drops every line of. It holds **980 976 frames of `0x102` and 14 presses**: 4 cancel at 0.170–0.229 s and 10 ENTER at up to 0.250 s, on 2026-06-14. No `capture-*.log` covers June at all — they begin 2026-08-02 — so those presses dedupe against nothing and are absent from any sweep that misses the file.

⚠️ **And a second file goes missing for an unrelated reason.** `capture-20260807-213342-1e1b5c60.log` ends in a run of 424 NUL bytes (488 in the file altogether), so `grep` calls it binary and prints nothing without `-a` — 177 frames of `0x102`, dropped in silence by exactly the sweep that was written to catch the first omission. **Use `grep -a`.** Two files, two different ways of being invisible, and neither announces itself; a sweep that reports a file count rather than a frame count cannot tell you it happened.
...
**`btn_mode_enter` is the only decoded handlebar bit in THIS archive with no long press anywhere.** Every other button that is held for anything reaches 1.2 s to 191 s — the 191 s being the jacket resting on `btn_mode_right` that `src/can/vcu-digitals.ts` already records. That, rather than anything about the fan, is why the fan cycle is on ENTER and not on either MODE arrow. ⚠️ The **ride log** is a second archive and it does not agree — four ENTER presses at or past 1.2 s, the longest 4.260 s — which changes no threshold and does change the sentence: §"Long ENTER presses in the ride log".
...
⚠️ **ENTER is not closed, and the two sweeps run for #192 do not agree on it either.** One counted 147 presses in the candump half, the other 150; adding the June file's 10 gives **157 or 160** against the 160 recorded here, so the residual is somewhere between **3 and 0** depending on whose edge pairing you take. **Neither was forced to agree with the other**, because a reconciliation nobody can reproduce is worth less than a disagreement with both rules written down. The 147 sweep's rule, stated so it can be re-run: read every top-level `.log`/`.txt` with `grep -a`; per file, emit the first `0x102` frame as a baseline and every subsequent change as an edge; open a press only on a **watched** 0→1, so a file whose first frame already has the bit set contributes no press from it; discard a press still open at end of file; then dedupe across files by absolute press start instant at 10 ms. 🚨 **An earlier version of this paragraph blamed the gap on "the 150 sweep's file-boundary handling". That is refuted.** Across all 268 files there are **0** files that open with either bit already down and **0** presses left unterminated at end of file, so the two clauses that differ between the sweeps never fire once; and ENTER comes out at 150 at **every** dedupe tolerance from 0 to 100 ms. Neither mechanism can produce 147. The 150 sweep reads raw bytes with no `grep` and no `can0` anchor, and its candump-only figures (150/775) reproduce the second implementation exactly while its whole-corpus figures (160/779) reproduce the ones recorded here exactly — which is a structural argument that 150 is right and 147 is a defect in the other sweep that nobody has yet located. **A third, independent sweep written for #208 lands on the same four figures**, so 150/775/160/779 is now reproduced by three implementations and 147 by one.

⚠️ So: **the counts stand at 147 and 150, and the cause of the difference is unknown.** What is ruled out is written above; the two sweeps agree exactly on cancel (775 each) and differ only on ENTER, which is itself the strangest part and the place to start looking. Recorded this way because the previous version named a mechanism that had not been checked, which is the error this section is otherwise about.
...
⚠️ **The per-button table above is the 268-file capture archive, and it is not the only archive on this bike.** Re-measured on 2026-09-10 against `rides.db` — a different corpus, written by the service itself rather than by `candump` — `btn_mode_enter` has **94 press→release pairs, 100 ms to 4 260 ms**, and four of them clear the fan gesture's 1 200 ms:
