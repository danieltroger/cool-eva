...
1. **The fingerprint.** `buildParameterTable()` fingerprints the rebuilt rows and throws when they disagree with the digest taken from Energica's own bundle, and `param-table.ts` builds the default table at module load, so the service would refuse to start. 16407's recorded digest is `5757d064`; appending the single row `278 A8_FW_278 DWORD U A8` to its 277 makes it **`8b80f344`**. ⚠️ That second digest is specific to that row — `fingerprintTable` covers index, name, width, signedness and micro, so any other row gives another number. The reproducible claim is that it CHANGES, and it changes for all 29 tables at once, because they are all deltas against the same base.
...
- **read-service-stamp** is read-only, and reads ids 1000-1003 — outside the name table's 1…277 entirely, from `SERVICE_RESET.md` §2 rather than from any table. What they answered when it was finally run: `docs/service-stamp.md`.
...
### ⚠️ Why an allowlist and not a range check on an arbitrary identifier — and why the allowlist went

⚠️ **This subsection argues for a design that was removed on 2026-08-21.** It is kept because the reasoning is still the reasoning; only its conclusion moved. The argument below is that a caller must not be able to aim at an arbitrary index — and that is still true and still enforced: nothing here has ever taken an index, the HTTP layer takes a name, and a plan whose index does not match its name is refused by the codec. What the five-name list added on top of that was RESEARCH, not addressing safety, and the owner asked for the other 264 to be reachable with that distinction shown rather than enforced by absence. Read on for the original argument.
...
2. **A caller cannot name an identifier.** `write-parameter` takes a `ParameterWritePlan`, and the ONLY way to obtain one is `write-targets.ts`'s `planWrite`, which refuses any name that is not a parameter of this bike's table, or one of the four whose name appears twice and so has no unambiguous address. There is no constructor for a plan in the codec and no field a caller could fill in by hand that it would honour — `encodeWrite` re-checks the plan against the allowlist on the way out (`assertPlanIsAllowed`).
...
- The single-frame length check: a WORD write is 5 bytes and a key send is 5, so nothing on the allowlist can reach it. It stays because the next parameter someone adds might be wider, and a silently truncated write to a calibration EEPROM is the worst outcome that file has — it would look like a success and leave a different number in the cell. Nothing there assembles a multi-frame request, on purpose. ⚠️ **That "next parameter" now has a name.** `ParameterStorageType` gained `DWORD` in 2026-09 so a 4-byte record could be READ; a `2E` write of one would be `2E <hi> <lo>` plus 4 = 7 payload bytes, and this is the check that refuses it. No table in the catalogue carries a DWORD row, so it is unreachable today — the point is that widening the type for the read path did not quietly make a truncated write expressible. `datatypeBounds` in `write-targets.ts` takes its width from `recordLengthFor` for the same reason: it used to answer 16 bits for anything that was not a BYTE, which would have offered a 32-bit cell a 16-bit range that looked researched.
...
Bank 2 (`0x2000 | index`, the RUNNING values as against bank 1's stored settings) had never been read. Swept on the bike with the service running, through `/vcu-probe` — so **no service stop is needed for parameter reads**. A freeze-frame read does need one today, but ⚠️ **for socket ownership rather than because the bike refuses**: `scripts/read-freeze-frame.ts` opens its own socket while the service holds one, and these micros answer on a single id with no request tag, so two testers are resolved by whichever frame lands first. Whether the service itself, as the single tester, can run that read in-process is **built and not yet proven**: `POST /lifetime-read` does it, parking the 2 Hz OBD poller for the duration, and the read measures its own dispatch latency so the first run on the bike answers the question with a number rather than an argument. `docs/lifetime-battery-statistics.md` § "Reading it in-service".

**A9 answers `1…197` and refuses `0` and `198+`** with NRC `0x31` (requestOutOfRange). That is a hard edge, not a scatter: every index from 198 up refused, and 197 answers. ⚠️ **This used to say `0…197`, and index 0 does not answer** — re-verified live on 2026-09-08: `0x2000` is refused with NRC `0x31` while `0x2001` reads `01 70` (368) and `0x2197` reads `00 00`. ⚠️ It also used to cite "150 consecutive reads answered", which does not correspond to 197 indices either way; the sweep behind that number was partial, and the figure is withdrawn rather than restated.

⚠️ **"A8 refuses bank 2 at the one index tried (258)" reads as though A8 has no bank 2, and that is wrong.** A8 bank 2 index 1 answers (multi-frame), consistent with this document's own §A record of `22 2001`. What index 258 shows is that _that index_ is out of range, not that the bank is absent. Bank-1 index 258 (`MAX_DC_CHG_CURRENT`) is therefore **past the end of bank 2**, which is why an early attempt at it returned nothing.
...
### ⚠️ A read's reply CAN be multi-frame, and since 2026-09-14 it is assembled

This section used to carry a paragraph headed **"Why single-frame only in `param-codec.ts`, and why that is not a gap"**. It is worth being exact about which half of it was wrong, because the heading is what gets quoted and the premise is what gets re-derived.

- ✅ **Its stated premise is still true.** A bank-1 record in `params.ecf` is 1 or 2 bytes (the TYPE column), so the longest reply those 277 parameters can give is `62 <hi> <lo> <b0> <b1>` = 5 payload bytes, and extended addressing leaves room for 6 in a Single Frame.
- ❌ **Its unstated premise was that the name table bounds the read path.** It has not since 2026-08-16: `probe.ts` reads any bank at any index, and `write-session.ts` reads indices 1000-1003.
- ❌ **So its conclusion — "and why that is not a gap" — was wrong**, and the bike said so. On 2026-09-14 a read-only probe of A8 bank 1 index 278 came back `{"status":"multi-frame","rawHex":null,"note":"the reply was a 7-byte multi-frame transfer, which nothing here assembles"}` (`evidence/probes-20260914.txt`). Seven bytes is `62 11 16` plus a **4-byte record**, which is what [#219](https://github.com/danieltroger/cool-eva/issues/219) types index 278 as from A8's own firmware parameter table.

**What changed.** `kwp-client.ts` now runs EVERY parameter read through `multiframe-transfer.ts` — the same transport the five multi-frame services use — rather than through its own one-frame-out/one-frame-back `exchange`.

⚠️ **No request byte moved.** `segmentRequestPayload` on a 3-byte `22 <hi> <lo>` payload emits `[address, 0x03, 0x22, hi, lo, 0, 0, 0]`, which is byte for byte what `buildRequestFrame` emitted, zero padding included. `scripts/check-vcu-params.ts` §2 asserts that over both micros and five request kinds, so the 277 reads a sweep does are unchanged on the wire. What changed is the reply side.

**Why routed rather than re-asked.** The cheaper-looking alternative — keep the single-frame path, and ask again through the multi-frame one when a First Frame turns up — was rejected. A micro that has sent a First Frame and had no flow control sits in its N_Bs window with its transmit side busy; a second `22` ten milliseconds later meets it mid-abort, and whether A8 answers that is a guess about its ISO-TP stack that cannot be tested from here. Answering the First Frame is instead exactly what 1229 of 1229 captured replies show working.

⚠️ **`param-codec.ts` still assembles nothing, and still derives no transmit address from anything the bus said.** That property is not spent, it has moved: `buildFlowControlFrame` addresses the target the CALLER named, and the transport never reads an address out of a received frame. A First Frame arriving at `parseResponseFrame` is still reported as its own outcome rather than decoded from its first fragment — a truthful "this did not fit the shape I understand" beats a plausible-looking value assembled from half a record.

### ⚠️ `write-session.ts` reads `22` too, and is deliberately NOT routed

`src/vcu/write-session.ts` has a second, independent `22` read path — its own `readIdentifier`, its own `exchange`, its own session — which still turns a First Frame into `the reply was a multi-frame frame`. It is reached by the compare-and-swap before every write, the read-back after it, and `runReadServiceStamp`, which reads **A8 indices 1000-1003** (`SERVICE_STAMP_IDENTIFIERS`) — inside #219's block of 25 parameters `params.ecf` does not describe. Three reasons it stays as it is:

1. **Its failure mode is the opposite one.** On the read path an unassembled reply means a parameter is unreadable, which is the silent-shaped gap this work closed. There it means the write is REFUSED, on a value that could not be read. That is correct behaviour, not a gap.
2. **`exchangeRaw` is the single transmit slot the SecurityAccess session shares** with writes, `0x27` and Mode 04. Teaching it a second transport means an undiscriminated pending slot becoming a union, and a flow-control send from its frame handler, in the file where a mistake writes to a calibration EEPROM. That is its own piece of work.
3. **It is not reachable today, and that is measured rather than assumed.** #219 types 1000-1007 as WORD → a 2-byte record → a 5-byte reply → one frame. The same 2026-09-14 probe run that got the First Frame at 278 read index **1000 back as `00 00`, single-frame**.

So `VcuFrame`'s `multi-frame` member in `param-codec.ts` is live rather than vestigial, and `write-session.ts` is its consumer. Filed as a follow-up rather than done here.

### What the read path now puts on the bus that it did not
...
- ⚠️ **`note` carries provenance on a row that ANSWERED, which is a special case on a shared field.** `VcuParameterRow.note` otherwise means "why this row is not a value"; for these 25 it also carries what is known about them, whatever the status, because a flat row has nowhere else to put it. Two costs, both measured rather than supposed: `retableRow` has branches that must not re-fold it (a three-copy note shipped once and is now guarded by a counting assertion), and `public/lib/params-page.js` renders `note` in the VALUE cell of a non-read row, so a silent block row's value column is a long sentence while a row that answered shows none of it. The deeper fix is a `known` field on the row, re-derived on every re-table exactly like `type` — duplication impossible by construction, and the page decides where to show it. Not done here: it changes the stored shape and the wire shape the dashboard types against, and it owes a render decision and a dashboard-gate pass of its own.
- **Diff noise, in three shapes, all honest and all worth expecting.** The first sweep after this lands reports 25 × `appeared`. A park-refusal sweep reports 25 × `status-changed` (`read` → `not-sent`) and the next good sweep reports 25 more going back. And ⚠️ **278/279 report `value-changed` after every ride** — they are the odometer. That last one will be the loudest line in most diffs, and it is true rather than noise.

**The third option, rejected — and the evidence that would overturn it.** Review proposed a transport-aware form instead of any predicate: read normally, and if a reply turns out to have been multi-frame where a narrow record was expected, read it again under a hold. It would cover the 277 as well, which the current rule does not. Against it: it puts a second read path and a retry rule inside the sweep loop, for a population whose widths have 233 live records behind them; and since #231 a surprise First Frame is already ASSEMBLED, so the residual failure is an honest `stalled` row that the next sweep retries, never a wrong value. ⚠️ It is also not something a park could have saved: the transport learns a reply spanned frames only after the First Frame — i.e. after the collision window has opened — and a hold takes up to 6 s against a 400 ms transfer timeout.

**The falsifier is specific and already carried**: a `stalled` row with a **non-null `flowControlLatency`** means a First Frame arrived where no width predicted one. That field is on every read, including the failure branches, precisely so this is visible in a journal. One in the wild turns this from a rejected option into a follow-up with a reason.
...
Three smaller changes on the same path, written down because they are the kind of thing that is puzzling in a journal six months later:

- **A flow-control frame arriving from a micro during a read** used to be handed back to the shared socket as `ignored`. It is now consumed, with one log line per transfer (`flow control from … with no request frames outstanding, ignoring`).
- **A MALFORMED First Frame — one short of 8 bytes, or declaring fewer bytes than a Single Frame holds — is now `abandoned` rather than `ignored`** in `extended-iso-tp.ts`. `ignored` means "not ours, hand it back", and a frame addressed to the tester is nobody else's; leaving it that way let the caller's window run out instead, which for a read meant timing out as `first-reply` and **re-asking** — putting a second `22` on a micro mid-ISO-TP-abort, the one thing routing reads here was chosen to avoid.
- **A read abandoned because WE stopped** — the gate shutting, a shutdown — reports `not-sent`, including when the stop lands on the retry's `10 81`. That used to come back as `no-session`, which `probe.ts` renders as "either nothing is at this address, or it is asleep": a claim about the motorcycle for something the motorcycle did not do. The sweep was insulated by `sweep.ts` discarding an in-flight outcome on abort; the probe was not.
- **Every read carries `flowControlLatency`** — null when the reply needed none, which is the ordinary case. Non-null exactly for the wide records, where it is the first number to look at if one comes back `stalled`, and it is carried on the FAILURE branches too, which is where it matters.
- **A read whose First Frame arrives and then stalls** costs `responseTimeoutMs + transferTimeoutMs` — 300 + 400 ms by default, against 300 before. Still well inside the micro's ~2.5 s session and the client's 1500 ms `SESSION_IDLE_LIMIT_MS`, and it is not retried: a stall means the micro answered, so the stale-session premise the one retry rests on is absent.
- **A reply is assembled under a 32-byte payload cap**, chosen here rather than inherited because `extended-iso-tp.ts`'s constructor asks each caller to state the largest reply its own service can justify. It derives two further caps — 6 contributing frames and 22 frames per exchange — so a read abandoned for frame count was over one of those. The declared length survives inside the abandonment's reason either way, so the cap costs no evidence when it fires.

**Two new outcomes, and why they are two.** A read can now come back `stalled` (a First Frame arrived and the rest never did — a claim about the micro or the link) or `abandoned` (a reply arrived and was discarded: sequence gap, short Consecutive Frame, declared length over the cap — a claim about our framing, or about something corrupting bytes on the bus). They replace the old `multi-frame` status. Folding them into one was proposed and rejected in review: in a garage with no reception they send you to different places, `multiframe-transfer.ts` and `VcuMultiFrameOutcome` already keep exactly that pair apart, and the service-mode page counts by status rather than by reason string.
...
- 🟡 The code proves it answers at `0x18DA09F1` in bootloader/programming mode. Whether the RUNNING APPLICATION answers there without a reset now has its **first datapoint, and it is a no**. On 2026-08-19 18:43, during a live (fault-free, non-DC — inferred AC) charging session, the armed CM probe sent four identification reads to `18DAF109` (device `0x09`): `0x22` on `F191`/`F181`/`F180`/`F18C`, each well-formed. **All four got no reply.** ⚠️ This does NOT refute the address: the decompiled service-tool code only ever demonstrates these reads in bootloader mode, so a running-app no-reply is exactly what it would predict, not a contradiction of it. And it aligns with an independent second source — a community member reports the charge manager (`0xA4`) ignores diagnostics in its running application on their bike too (PR #77). So the running charge manager does not appear to answer identification reads live, on two bikes; whether it answers ANYTHING live, or only after an ECUReset into the bootloader, is still open — and an ECUReset must not be sent during a charge session, which is what the probe correctly declined to do. The BMS and PSU precedent (same node for app and boot) still makes the address itself likely.
...
⚠️ **`exchange` still exists, but a parameter read no longer uses it.** Since 2026-09-14 it carries only `10 81` and `3E`, whose replies are one and two bytes; reads go through this transport because a record can be 4 bytes wide ([§9](#9-the-read-path-read-only-by-construction)). `0x22` was deliberately NOT added to `VcuMultiFrameRequest` to make that work — `kwp-client.ts` hands the transport a payload built and allowlisted by `param-codec.ts`'s own union instead, so both unions keep the sizes this section states as invariants: three members and three service bytes there, five and five here.

- RECEIVE: a `0x17` freeze frame is at minimum a 5-byte header plus fields, and an extended-addressed Single Frame holds 6 bytes — so every freeze frame is multi-frame. Assembling it means ANSWERING the First Frame with a flow-control frame, which the parameter-read path was built never to do and now does, through this module rather than in `param-codec.ts`.
...
- **`rolling_backwards`** (`0x104` bit 63, and it shipped as **`reverse_gear`** until 2026-09-14 — #216 renamed it because it is not a gear; everything below still holds, only the key moved) — the obvious "not in gear" check, and now the best-understood rejection here. Settled against `rides.db` on 2026-08-16, six days of riding:
  ...
  ✅ **Updated 2026-09-14. The rate statistics above reproduce on a seventh day, and the guess about the meaning was right.** Against the decoded ride log of 2026-09-13 (`cool-eva-2026-09-13.celog`; the day holds 3 709 811 readings over sessions 138-147), 240 clock-clean assertions give median `speed_can_kmh` at a rising edge of **0.3 km/h**, p95 **0.9**, max **1.2**, with **0 of 240 above 4.5** — against the 0.4 / 0.7 / never-above-4.1 measured here on `rides.db`. It strengthens to a form this entry did not have: the bit is **never set above 4.9 km/h at any moment**, not merely at the rise. And the manufacturer's own frame table names the field `V_SPD_DIR`, _speed direction_ — so "direction-of-rotation or rollback indicator" was the correct reading and is no longer a guess. 🟡 The **polarity** is still not established by anything in this repo: the table gives no sense, and with both speed keys unsigned no witness in the log can tell backwards from forwards directly. #216 reports a firmware read giving the bit as `(signed D_MOTOR_SPD < 0)` past a ±500-count deadband with no hysteresis; that is attributed, not reproduced here.

  ⚠️ **But ONE HALF OF THE REJECTION REASON ABOVE IS NOW WRONG, and it is left standing rather than quietly rewritten.** "A 50 ms pulse cannot gate anything without heavy debouncing" assumed a pulse. It is a comparator output, and **33 of those 240 assertions last ≥ 1 s, the longest 17.930 s** — durations any gate could sample. The rejection still holds, but on the _other_ half only: "the wheel is creeping" is already covered by `speed_can_kmh`, which `src/vcu/service-gate.ts` requires to be exactly zero. Nothing about the gate changes; what changes is that a future reader must not cite the debouncing argument, because it is false. Full measurement: `docs/can-decode-findings.md` §`reverse_gear`.
...
- **`0x101` b0/b1** (decoded since 2026-09-14 as `vehicle_substate_can` / `vehicle_state_can`; `docs/can-0x101.md`) — suggested as a vehicle-mode enum for the service gate. Decoding it has not changed that answer: `obd-garage/DC_CHARGE_LIMITS.md` §5 gives 43/40 for **"riding / idle" as a single row**, 62/60 transitional and 104/100 DC charging, from one session on 2026-08-04. It therefore does not separate a moving bike from a parked one — it is a not-DC-charging detector. It is also not decoded here at all, not in the kernel RX filter, and `CAN_MAP.md` (older) still lists `0x101` under "unmapped".
...
**What it actually does, beyond clearing the list.** 🟡 Mode 04 in the standard also resets readiness monitors and the freeze frame, and on many ECUs it resets fuel trims and adaptive values. Five counters were measured moving on 2026-09-13 — see below — but the full extent is still not known. It is not a "clear the dashboard light" button; it is "erase the diagnostic memory", and the honest position is that we do not know the full extent of what is erased.

**It works, it has twice not worked, and what the button does about that is its own document.** ⚠️ On 2026-09-13 a press swept 41 of 46 codes and moved five counters, with the raw `01 04` / `01 44` exchange captured. On 2026-08-08 and 2026-09-11 the identical press was answered `44` and erased **nothing** — the stored count never moved and PID 31 kept counting from 19 173 km — and both were journalled "Mode 04 accepted". The clear now parks the OBD poller, reads the bike back on both sides of the frame, and judges the result on PID 31 rather than on the reply byte. The captures, the counters, the charger hypothesis and the cache bug that hid all of it: **`docs/clear-dtcs.md`**.

**Telling our Mode 04 reply from the poller's traffic.** `isClearDtcsReply` exists because the OBD poller holds no bus lease — the lease excludes the two service-mode runners from each other, not the poller — and `startObdPoller(500)` keeps sending mode-01 PID requests and, every 120th round, a multi-frame mode-03 transfer. So the OBD response range carries other people's frames while we are listening, and "the first frame in `0x7E0`-`0x7EF`" is not our answer.

⚠️ **This is no longer the ordinary case, and it used to say it was.** Since `src/vcu/clear-dtcs.ts` the only Mode 04 caller parks the poller for the whole exchange, so the bus is quiet for the 300 ms window. `isClearDtcsReply` stays as defence in depth: `holdObdPoller`'s cap is enforced by the loop, not by the holder, so a read-back that overruns 15 s finds the poller running again underneath it.
