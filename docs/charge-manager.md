...
**b2 `charge_type`** — 1 = AC, 2 = DC. 99.991 % of the 151 200 frames where DC current is flowing read 2 (the 2026-08-19 figure was 99.975 % of a 43 994-frame sample). It never names the _wrong_ mode. ⚠️ But it is emphatically NOT a stable "a session exists" flag, and code that treats it as one breaks: **on AC it flaps 1↔0 within a single plug-in as the charger pauses delivery.** In the 2026-08-25 01:45→03:48 AC trickle session (one continuous plug-in, mains steady ~210 V/1.5 A throughout) `charge_type` toggled 1↔0 fourteen times, reading 0 for stretches from 1 s up to a full **8 minutes** (01:58→02:06 and 03:00→03:08) while current was still flowing. The earlier DC datum (aborted 2026-08-09 attempt: reads 2 for the whole 139.3 s while not one amp moves) said the opposite because DC holds it steady — but AC does not, and the difference is not documented anywhere in the DBC. **For "is there a live session", use `charge_manager_state` (b7 of `0x610`), which held a steady `0x02` across that entire two-hour AC session; for "is charge actually flowing", use `dc_charging` / `ac_charging`.** This flap is why the dashboard charge-current control (`public/views/charge-current.js`) and the Pi runner (`src/vcu/write-runner.ts`) key session presence on `charge_manager_state`, not `charge_type` — see `docs/dashboard-decisions.md`.
...
The bit that _does_ track it is **b3 bit 0, frame bit 24 — `fast_dc_contactor`, already decoded** (`V_FASTDC_MON_SW` in `src/can/vcu-digitals.ts`), which agrees with `0x605` b7 in 99.39 % of 9 890 746 aligned frames. That is the same fact arriving by the physical route rather than the command route, and it is the one to use if you want the DC state without the charge group awake.

🟡 Incidental correction to `src/can/vcu-digitals.ts`: its note that `0x102` b3 bit 2 "is never once clear in 1 103 000 frames" is true of its 14-capture sample and not of the archive — b3 reads `0x40`/`0x41` in **279** frames, so bit 2 is set in 99.998 %, not 100 %. ⚠️ This line said **278** until 2026-09-14; the pass behind it predated seven of the archive's candump logs, and `0x41` contributes 34 of the 279. The bit is decoded as `dsb_control` since that date and the two windows are characterised in `docs/can-decode-findings.md` §"0x102 byte 3".
...
⚠️ **SUPERSEDED by the 2026-09-07…13 log — see `docs/dc-taper.md` § "The 73 A cap is not a law any more".** `fast_dc_target_a` reads 74 once and 75 twice there, and both 75s are held rather than ramp transients: 6.4 minutes on 2026-09-12 with `pack_a` at 74.7-75.0 A, and 4.8 minutes on 2026-09-13, both at a station whose `fast_dc_limit_max_a` read 80 rather than the usual 75. The paragraph below remains a true statement about the corpus it was measured on, and its "start at that gap" advice is now answered: the gap closes when the station advertises more than 75.
