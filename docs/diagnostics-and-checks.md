...
**No bike — and no waiting for one.** Nothing in the suite opens a CAN socket. `socketcan` is imported at runtime in exactly one place, `src/can/socket.ts`, which none of these reach; everywhere else it is `import type`, which type stripping removes before Node ever sees it. That is why the suite runs on macOS and on an Actions runner, neither of which has a `can0`. `CHECK_TIMEOUT_MS` (120 s) is the guard against that quietly changing: a check that did reach for hardware would mostly not fail — it would sit waiting on a bus that is not there — so a check that stops producing a verdict is counted as one that failed. The whole suite runs in about **95 s** of wall clock on a laptop and about **18 s** of CPU — measured 2026-09-19 at 97.2 s / 17.7 s, and 2026-09-15 at 92.6 s / 16.1 s before #286 — so it is mostly checks sitting still, waiting out kick-starts and staleness windows in real time. The slowest three are `check-fan-curve.ts` at 6.8 s, `check-fan-fun.ts` at 7.9 s and `check-fan-race.ts` at 4.8 s. ⚠️ The first and third grew there in #286, by 2.8 s and by a second: `check-fan-curve.ts` §10's closing ramp waits out a second real `KICK_START_MS` so that it can read the bridge rather than the bookkeeping, and `check-fan-race.ts` §7b builds a second wedged controller. Both are sleeps, not work — `KICK_START_MS` is a module constant no check can turn down, which is the one lever that would give the suite about five seconds back. ⚠️ This said "under ten seconds … on a tenth of a second of CPU" until #266, which is wrong by an order of magnitude on both — a reminder that a number in prose is only as good as the last time somebody measured it. The timeout is per check, not for the suite, and is still not a performance budget.
...
| `import-ride-log.ts` | the same, and then some: it spawns that decrypt, spends ~20 minutes and tens of GB of heap on it, and finishes by **replacing the live `rides.db`**. What CI checks is the part that can be checked — `check-import-ride-log.ts` drives the orchestration with the two child processes injected, so every refusal and every file move runs in milliseconds against tiny real databases, and `check-route-track.ts` runs the materialiser it calls |
...
| `check-phone-width.ts` | **needs a browser.** It is the one check that opens a page, and `npm test`'s claim is that it runs anywhere with no bike and two devDependencies. It is not skipped, it is MOVED: `.github/workflows/test.yml` runs it as its own step, on a runner image that ships Chrome, so every PR is still gated on it. §11.8 |
| `headless-page.ts` | not a check — the CDP plumbing §11.8 is built on |
...
**`scripts/check-ble-retry-policy.ts`** — **no fixture file; one measured constant.** `LONGEST_EPISODE_MS` is the real 11 h 42 min wedge of boot `2026-09-17T18:42:47` (18:53:36 → 09-18T06:36:02, 8 206 lines). ⚠️ An earlier version of this paragraph named a second one, the 35-57 s from the last good connect to the first storm line — that measurement is real, but the rate it was used to derive was wrong and is retracted in `docs/ble-adapter-wedge.md`; the cooldown rule now rests on the constant's own semantics and turns on no measurement at all. It is re-derivable from `cool-eva-route/data/pi-journals/journal-all-boots.short-iso.zst`; `docs/ble-adapter-wedge.md` carries the tables and the queries. The policy is pure and takes `nowMs`, so the whole episode replays in milliseconds — which is exactly why §9 of that check reads `src/ble/client.ts` as TEXT: a synthetic clock passes whether the caller hands it `monotonicNow()` or `Date.now()`, and this Pi steps its wall clock from GPS.
...
**`scripts/check-button-decode.ts`** — 25 frames are REAL, copied byte for byte with its timestamp out of the candump captures in `~/Documents/cool-eva-archive` (see `CAPTURES.md` there), plus **2 marked ⚠️ SYNTHETIC** since 2026-09-14. A hand-written frame only proves the decoder agrees with whoever wrote the fixture, so those 2 are used only where nothing real can do the job: four of byte 3's bits (`V_HORN_SW`, `V_IMD_DISABLE`, `V_WINTER_STORAGE`, `V_ABSOFF`) are 0 in all 15 006 856 archive frames, so no captured frame can pin their positions or stop two of them being swapped. Same treatment, and same reason, as `check-freeze-frame.ts`'s 2 constructed transfers below. The `0x400` button payloads in particular are the only ones ever recorded on this bike: across **1 099 357** frames of `0x400`, byte 2 held a non-zero value in **362** of them and took exactly two values — until 2026-08-19, when a session of deliberate presses finally produced a third (`0x01`, `btn_set_back`, 132 frames).

**`scripts/check-wifi-diag.ts`** — the wifi logging and the dump builder behind `docs/wifi.md`. Its fixtures are `nmcli -t` output captured off the Pi rather than invented, because both parsing traps are invisible until you see real output: `-t` escapes the separator **inside** a value, so a BSSID prints as `CC\:BA\:BD\:34\:31\:92` and `split(":")` reads six fields out of a three-field row — an SSID may contain a colon too, which a hotspot rename can produce at any time — and `GENERAL.STATE` is a number **and** a word (`100 (connected)`), so `Number()` of the whole value is `NaN`. The state fold is checked against libnm's own constants read out of `nm-dbus-interface.h`: `FAILED` (120) and `DEACTIVATING` (110) are DISCONNECTED and **not** UNAVAILABLE, because the radio worked throughout the 2026-09-19 silence and calling that "unavailable" names the wrong fault. Then the reading the feature turns on — the hotspot **is** in range and we are **not** on it — which neither half says alone. The dump half: a fixture **carrying** a real-looking PSK must come out with that string absent while the setting it sat on is still visible (a redaction test whose input has nothing to redact passes with the redactor deleted), and a failed, timed-out or truncated command must say so rather than looking complete. Then the rail that a dump is safe to take at any moment — every collected command is a read, no `connection up`, no forced rescan, never `--show-secrets`. Finally two contracts invisible from either side alone: each `wifi_*` key's bound resolved from its **registry entry** rather than spelled, so moving the group back to `diag` (where a blank-unit code is gated to `[0,1]` and `wifi_link_state = 3` draws as a dead sensor) turns it red; and the poll interval read off `src/http/status.ts`'s own exported `FRESH_MS`, since a `source: "poll"` signal slower than that window reads dark on a healthy Pi. **30 mutations, all red by exit code** — including the group move, the un-escape, the 120-fold, `Number()` on the whole state field, a pass-through redactor, dropped stderr, an unannounced truncation, `--show-secrets`, a forced rescan, the poll back to 30 s, and `bounds` dropped while the group stays `wifi` (which the bounds generator itself refuses, proving the fail-closed arm).
...
Its thresholds are checked against the whole 268-file archive rather than the 14-capture corpus above: **290 ms** is the longest MODE ENTER press in that capture archive, over 160 presses, and the 1200 ms fan hold clears it by 4.1× (⚠️ the **ride log** is a second corpus and does not agree — four ENTER presses at or past 1200 ms, the longest 4 260 ms; `docs/handlebar-gestures.md` §"Long ENTER presses in the ride log"); **330 ms** is the longest `btn_indicator_cancel` press outside one afternoon's experiment, over 770 of 779, and the **500 ms** waypoint hold clears it by **1.5×**. The full per-button table, the nine presses inside that one minute, the 500-607 ms band the beat actually produces, and the method's failure mode are in `docs/handlebar-gestures.md`.

**`scripts/check-fan-banner.ts`** — the banner that hold raises, which is a different question from whether the hold fired. The fan's mode and its duty are published by two different files and the banner is worded from **both**, so they are a pair that can tear: #199 said _"Fan: manual 68 %"_ over a fan at 100 %. The mechanism, the eleven measured instances and the invariant are `docs/fan-control.md` §"The two fan signals must reach the phone duty-first".

⚠️ **It replays the real patch sequence and does not sample.** `onChange()` is subscribed the way `src/ws.ts` subscribes, and each batch goes through `public/lib/store.js`'s own `apply()` and `public/lib/announce.js`'s own `foldFanAnnouncement()` — because sampling `latestValue()` after a settle is precisely what cannot see this bug: both signals are correct a few milliseconds later, and it is the arrival **order** that lies. Three properties, each pinned by its own mutation: the publish order in `src/fan/auto.ts`, the mid-kick publish in `src/fan/control.ts` (which no re-ordering fixes), and the guard that stops the deferred publish stamping MANUAL over a decision the curve made during the command.
...
⚠️ **This paragraph used to date the clear `2026-08-09 14:42:32`, and that was wrong twice over.** The archive holds exactly **one** `7C0 … 14 FF FF` request in every capture on disk, and it is on **2026-08-08**. There are zero on 2026-08-09 — the 27 hits a naive `grep '14 FF FF'` finds that day are payload bytes inside broadcast frames — **24 in GPS `0x410`, 2 in `0x027`, 1 in `0x600`** — and not one is a request on `0x7C0`, which is presumably how the wrong date was arrived at. Worse, `2026-08-09 14:42:32` is a **real timestamp for a different event**: it is when a charge-manager fault cleared during the aborted DC attempt (`docs/charge-manager.md`). Two unrelated "clears" a day apart had been conflated.
...
1. **The scoping was positional.** A close-up now finds its block by a marker INSIDE it (`PANEL_BLOCK`), and a marker moves with the block it names.
...
3. **The failure was mute.** One unchanging "timed out waiting for the sheet to settle" names neither what was awaited nor that a shape assumption, rather than a slow render, was what failed. `settle()` now takes what it is waiting for and says it, and a moved child count throws instead of timing out.

#### What the repair had to be argued into, three times

None of the three guards below was in the first version of the repair. Each exists because a reviewer put the bug back through it.

**The assertion is on the COMPUTED result, not on the marker.** `assertScopedToOneBlock` asks the browser which blocks are actually visible and whether the visible one is the one the marker names. A version that only counted marker matches passed with **every panel showing all six blocks** when the generated rule's `:nth-child` was off by one — the 2026-08-27 bug, reintroduced silently, under a green check and a passing assertion. Everything between a good marker and what a reader sees can fail quietly, `:has()` being unsupported included, and only the computed answer sees all of it.

**The assertion runs again after the staging, and re-queries.** Asserting once, before the panel is driven, leaves the very thing this repair is about unguarded: every staging step can re-render, and the write button's first tap re-reads the Pi and replaces the whole controls binding. Reverting only the stylesheet — back to `style.display`, marker unchanged — passed the check, passed the assertion, rendered panels 2-5 correctly and left **panel 6 showing all six blocks with no error at all**, because by then its inline styles were on nodes that no longer existed. The scoping is now asserted at the end as well, on a freshly queried node, since a reference taken at staging time is detached by the time it matters.

**A marker has to be written exactly ONCE in the view.** The obvious marker for the parameter form was `select.probe-input`, and it is the wrong one: that tag-and-class pair is built twice in `public/views/vcu-write.js` — the parameter picker and the bits value control — so renaming the picker a close-up actually holds left `check-service-preview.ts` green while both form panels died at render. The marker is `div.readout`, which is written once (the sheet's only un-editable field-shaped thing), and the check counts occurrences rather than merely finding one. **Uniqueness is what makes a rename unmissable**, and a marker that stops being unique fails with an instruction rather than going quiet.

**What is still uncovered: whether a panel renders at all.** That needs a browser, and `npm test` has none — not for the reason §11.1 gives about test frameworks, but because a headless browser was a dependency tree and a second runtime for a design tool nobody deploys, against a suite whose whole claim is that it runs anywhere with no bike and two devDependencies. The trade is deliberate and it is the reason this section exists. ⚠️ **Amended 2026-09-14 (#253):** the repo now does drive a browser, in `scripts/check-phone-width.ts` — with no new dependency and outside `npm test`, which keeps the trade above intact. It measures the WIDTH of the whole-dashboard preview, not whether an annotated panel mounted, so the gap this paragraph is about is still open; §11.8 says what it does cover and what it costs. What `check-service-preview.ts` covers instead is the **rename**, which is the likeliest way to break this again, and it now counts `form` panels — its `kind:` alternation had been silently omitting them, so a floor of five was being cleared by five of seven. ⚠ Adding `form` without moving the floor would have made that guard WEAKER than it was: seven counted against a floor of five passes a sheet with two panels deleted. The floor is what the sheet declares today, so a panel removed on purpose edits the check and one lost by accident goes red. Until the suite can open a page, the annotated preview is checked by a person looking at it — `npm run preview` builds the whole dashboard, `npm run preview -- --annotated` the design-review sheet — and this is written down so the next reader knows what the ✓ means. The page stamps `data-preview-failed` on `<html>` with the number of panels that did not render, so "did it work" is one ctrl-F rather than a scroll.
...
Two assertions, in two modules — `check-preview-fixtures.ts` asks whether the payloads are right, `preview-endpoints.ts` whether anybody answers the door:
...
Two more fields fell out of it on the first run: `READ_STATE.run.tally.byStatus` carried three of the seven statuses `tallyOf()` seeds at zero, in both templates, and `stepSweep()` was re-adding `run.expected` to a finished sweep at runtime — the same invented field, put back by code the check cannot see.
...
`src/ws.ts` sends a full snapshot every `HEARTBEAT_MS`; the old stub sent one and went quiet. `lib/connection.js` declares a link dead after `SILENCE_LIMIT_MS`, reconnects after `RECONNECT_DELAY_MS`, and the new socket sent one more and stopped — so the header cycled live → offline → live for ever. Measured on main's build over 80 s: **the header read `offline` in 32 of 200 samples, with `.stale` tiles in exactly those same 32** — and in a background tab the same page measures 33.7%, because Chrome throttles the timers `connection.js` polls on. That is the whole gap between the two figures this was first reported with, and it means the instrument was at its worst in exactly the situation a screenshot is taken from: a tab nobody is looking at. `isStaleWith()` calls every signal stale while the link is down, so roughly one screenshot in six taken off the instrument this project gates dashboard merges with was a picture of the dashboard's dropout appearance. With the heartbeat: 155 of 155 samples over 62 s read `live`, zero stale tiles.
...
A `<link rel="icon">` copied from `public/index.html` silences the last console line: Chrome's automatic `/favicon.ico` request, which over `file://` surfaces as an `Unsafe attempt to load URL` error. "The console is clean" is only a usable gate if it can actually be reached — it now is on the whole-dashboard preview, on all three scenes, on all five tabs and on the sheet. The annotated sheet answers `/fan` with a **404**, which is what a Pi with `FAN_ENABLED` unset does and what `views/fan.js` documents as "not an error": no panel there renders a fan section, so serving a live `FanReply` into it would be inventing a fan to hide.
...
**What it runs.** `scripts/build-service-preview.ts` builds the committed template against the shipped `public/`, i.e. the same file the dashboard gate looks at, into a temp directory. Then one page at 390×844 DPR 3 with mobile emulation, and **every tab of every scene** — 25 combinations, the scene list read out of the built page by `scripts/preview-scenes.ts` rather than restated, so a sixth scene is swept without anyone remembering to add it. All five scenes because four of the five tabs render almost nothing off the parked bike the `faults` scene inherits, and a width guard blind to the panels it is guarding is the failure `app-preview-template.html`'s own header warns about.

⚠️ **Each combination is a REAL page load, and getting that wrong is silent.** The first version navigated to `…?scene=faults#<tab>` — a fragment-only URL, which is a _same-document_ navigation: the page is not reloaded, `readyState` is already `complete`, the wait does not wait, and the measurement can belong to the previous tab if the re-render is a frame late. Proved rather than assumed, with a marker set on `window`: it survives `#faults` → `#ride` and is gone the moment the query string changes. Hence the inert `&tab=` parameter — the template reads only `scene` — whose whole job is to make each URL differ outside the fragment.

**Zero new dependencies, which is the whole reason it is allowed to exist.** `scripts/headless-page.ts` speaks CDP over the `ws` that already ships for the dashboard socket, and the browser is whichever Chrome or Chromium the machine already has — `$CHROME_PATH` first, then the usual paths. There is no Puppeteer, no driver, no download step.

**It never passes by default.** No browser is a red exit and the message says so; a selector that stops matching throws rather than narrowing; the fixture's long row is pinned BY ITS TEXT, because a Faults tab with nothing long on it would fit trivially and report a clean ✓ for a page nothing had tested.
...
It is closed the only way it can be: **from inside the sheet**, on ONE scene (it is the same sheet behind all five), with seven assertions and a probe.

- ⚠️ **The `.menu` click is an assertion, not a convenience.** `.sheet` is `visibility: hidden`, not `display: none`, so the sheet and every row in it have full layout boxes whether or not anything opened it — every one of the seven passes on a sheet nobody opened. So `open` is checked first, off `.sheet.open`.
...
- **And a synthetic probe**, for the same reason the stored-codes tile gets one. Nothing a waypoint row can really hold is an unbreakable token — a coordinate has a space in it and every sentence in `WAYPOINT_REFUSAL_TEXT` breaks at spaces — so no fixture can falsify `.waypoint-body { overflow-wrap: anywhere }`. Energica's identifiers are exactly that shape, so the probe appends a 145-character one and measures **the scroll container**: `.sheet-body.scrollWidth` reads 969 without the rule and 390 with it. ⚠️ **Not the row's box**, which is 355 px either way — #253's `.tile { min-width: 0 }` was copied here first and measured to do nothing, because a tile is a grid item whose track was sized by its content while a row is a block inside a block. A probe on the row would have been an assertion that cannot fail, which is how that line came to be deleted.
...
**What the copies had actually become.** The issue that asked for this recorded 653 identical lines out of 708 and 681, and three occasions when the drift had bitten. By the time the merge happened both files had roughly doubled and the numbers were quite different: **665 identical lines in order** — `difflib.SequenceMatcher(None, app, service, autojunk=False)` over the harness regions, trailing newline stripped — out of 1 432 and 1 163. That is 57 % of the smaller harness, not 92 %. Of the 18 top-level blocks present in both files, every one was identical once whole-line comments were stripped; eight more differed by a handful of code lines each; only `SCENES` and the route set of `previewFetch` genuinely diverged.

**And there were seven live disagreements, not three.** None of them was visible to `npm test`, because none is a fixture type, a scene reading, an endpoint-coverage gap or a parse error — which is everything the two preview checks look at:

1. **`clear-dtcs` was two different fixtures.** The annotated sheet carried the `?clear=nothing|retry` endings with the real 2026-09-13 numbers (46 → 5 stored, PID 31 19 671 km → 0) and a `clear` object; the whole-dashboard page still returned one sentence and no `clear` object at all. One page was updated; the other was not.
2. **`/vcu-read`'s sweep-start tally.** The sheet sent `byStatus: { read: 0 }` where the dashboard sent all eight statuses — under a ⚠️ _both_ files carried on `READ_STATE` saying "All eight, because `tallyOf()` seeds every status at 0 — the Pi never sends a partial one". The sheet contradicted its own comment. (`/lifetime-read`, next door, was byte-identical in both; the two are easy to confuse and the distinction matters to anyone re-deriving this.)
3. **`stepSweep` lost `expected` on one page only.** The dashboard deleted `run.expected` when a sweep finished, because `VcuReadState`'s `finished` arm does not carry it; the sheet did not, so its finished run was a shape the Pi cannot produce.
4. **The sheet broadcast `range_km` in the wrong group** — `battery`, against `src/can/registry.ts`'s `energy`. Invisible because `check-preview-fixtures.ts`'s `checkReadings()` reads `PARKED_SIGNALS` and `SCENES[].signals`, and the sheet had neither: it hand-wrote a 45-line `snapshot()` of 18 signals instead. This is the _same_ `range_km` mistake that check was written to stop, sitting in the one file it could not see.
5. **`const __TABLES = __TABLES__` was dead on the sheet** — declared, never read. The copy brought the declaration and left the route behind.
6. **`location.hash`** was missing from the sheet's `__previewLocation`, because the annotated harness never ran the router. The dashboard's copy carries a ⚠️ about it.
7. **The `.pv-stage` link filter** existed on the sheet and not on the dashboard, where the class does not exist and the filter therefore matched nothing — so "Open the full parameter table →" navigated out of the preview to a `file://` 404. Also ⚠️-documented, in the copy that did not have the bug.

Only 6 and 7 are real differences between the pages; they are now the parameter `INTERCEPT_LINKS_WITHIN` and one unified `__previewLocation`. The other five were one page being right and the other being old.

**Why one injected harness and not one parameterised template.** Outside the harness the two files share almost nothing: the dashboard page is about 6 lines of body and 32 of its own `<style>`; the sheet is about 240 lines of `<style>`, 30 of prose markup, and a 357-line panel renderer the dashboard has no use for. Their `SCENES` are different objects for different jobs — five scenes with whole signal tables against three gate-only ones. One template would have been some 1 400 lines of conditionals, and `check-service-preview.ts`'s `whole === annotated` assertion would then be guarding a flag rather than two pages.

**The contract, and why it is an ordering.** The parts are concatenated into ONE script scope, so the injection order in `scripts/preview-harness.ts` is the temporal-dead-zone contract and nothing in the files themselves would tell you so. Each template declares six names above the placeholder — `__TABLES`, `__MODULES`, `SERVER`, `SCENES`, `INTERCEPT_LINKS_WITHIN` and a hoisted `pageFetch` — and each one is there because it _cannot_ move into the harness: three are builder placeholders, `SERVER` must precede the page's `SCENES` (which reads `SERVER.*`), `SCENES` is the page's own bikes, and `pageFetch` is the page's own endpoints. `previewFetch` answers in one fixed order — shared routes, the generated tables, `pageFetch`, then the external-URL rejection and the final reject — so a page can neither shadow a shared route nor be reached by one.

`GATE` is a plain object literal that each page MUTATES rather than rebuilds, and both halves of that are forced: `check-preview-fixtures.ts` splices the declaration into `WRITE_STATUS`, `READ_STATE` and `LIFETIME_READ` to type-check them, so it must be a literal — and those three capture the same reference, so the scene has to be applied over it. A scene therefore states a DELTA or nothing; the annotated sheet's `parked` used to restate the default value for value, which is a second copy a change to the default would have left behind.

Two things stayed in the templates on purpose. `gateOf` is a `const` arrow used only by the dashboard's `SCENES`; as a hoisted `function` it would be invisible to `check-preview-fixtures.ts`'s `topLevelDeclarations`, which collects `VariableStatement` alone, and `gate: gateOf(SERVER.ridingGate)` would emit an unresolvable call into that check's throwaway `.ts`. And each page applies its own scene overlays as one contiguous block: hoisting just `Object.assign(GATE, SCENE.gate)` into the harness while its five siblings stayed behind would have re-created this section's bug class one layer down.

**What the sheet gained.** It now inherits `PARKED_SIGNALS` (40 signals rather than 18) and the `SERVER.heartbeatMs` heartbeat in place of a one-shot socket, which is what fixes drift 4 and what puts its signals under `checkReadings()` for the first time. Why the heartbeat cannot race its `settle()` waits is in §11.7's amendment, stated once rather than in both places. `liveSocket` also became a `Set`, because the sheet mounts one dashboard instance per panel and holds **six** sockets (one `renderSheet`, five `renderWriteSection`; the `key` panel never calls `instantiate()`), and a single slot kept only the last — so a waypoint saved in the first panel delivered its patch into the sixth panel's store.

**Why `seed()` takes the caller's word on connecting.** `store.connect()` is not idempotent: a second call builds a second link over a second socket, the store then applies every heartbeat twice for ever, and the first socket's interval keeps a connection the page can no longer see alive with it. The dashboard's entry point connects for itself when it imports the app, so it passes `false`; the sheet has no such entry point and passes `true`. This was eight lines of comment in one template and absent from the other, which is exactly the shape §11.9 exists to relocate.
...
- **It does NOT bring up `can0`**, deliberately, because `bringUpCan` may take the interface DOWN and that kills every other socket on it. ⚠️ **Read the link before typing any of this.** Stopping the service does not take it down, so on an `OBD_ENABLED=1` Pi it is normally still up ACTIVE and these three commands are unnecessary; on an `OBD_ENABLED=0` one it really is listen-only and they are exactly what you need. ACTIVE is the **absence** of `LISTEN-ONLY` from the `can <…>` group, not a field that says so — [`docs/can-capture.md`](can-capture.md) §"Reading the link by hand". If you do run them, the capture loses only the seconds the interface is actually down:
  ```
