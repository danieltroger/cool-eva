...
**`scripts/check-fan-banner.ts`** — the banner that hold raises, which is a different question from whether the hold fired. The fan's mode and its duty are published by two different files and the banner is worded from **both**, so they are a pair; `src/can/signals.ts` flushes one batch per microtask, so two records separated by an `await` are two WebSocket patches, and a mode that arrives ahead of its own duty is worded from the duty of the mode before it. That is #199: a hold that said _"Fan: manual 68 %"_ over a fan at 100 %, and off a cold pack said _"Fan: off"_ over a fan going to full.

⚠️ **It replays the real patch sequence and does not sample.** `onChange()` is subscribed the way `src/ws.ts` subscribes, and each batch goes through `public/lib/store.js`'s own `apply()` and `public/lib/announce.js`'s own `foldFanAnnouncement()` — because sampling `latestValue()` after a settle is precisely what cannot see this bug: both signals are correct a few milliseconds later, and it is the arrival **order** that lies. It is also why `foldFanAnnouncement()` exists as a function at all: the assertion this replaces fed the same percentage to the key and to the text, so it asserted a pair that cannot come apart and stayed green through the whole defect.

Two situations that look identical and fail for different reasons: a hold against a **settled** curve duty (the publish order in `src/fan/auto.ts`) and one against a duty still inside its 1500 ms **kick-start** (the missing publish in `src/fan/control.ts`, which no re-ordering fixes). Reverting either turns a different subset red.
