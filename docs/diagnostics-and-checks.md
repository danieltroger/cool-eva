...
**What the sheet gained.** It now inherits `PARKED_SIGNALS` (40 signals rather than 18) and the `SERVER.heartbeatMs` heartbeat in place of a one-shot socket, which is what fixes drift 4 and what puts its signals under `checkReadings()` for the first time. The heartbeat cannot race its `settle()` waits: `settle` polls a DOM predicate on `requestAnimationFrame`, and `assertScopedToOneBlock` runs synchronously when an `await settle(...)` resumes on a microtask — a `setInterval` callback is a macrotask and cannot interleave there. `liveSocket` also became a `Set`, because the sheet mounts one dashboard instance per panel and holds **six** sockets (one `renderSheet`, five `renderWriteSection`; the `key` panel never calls `instantiate()`), and a single slot kept only the last — so a waypoint saved in the first panel delivered its patch into the sixth panel's store.
