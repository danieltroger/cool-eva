...
Its thresholds are checked against the whole 268-capture archive rather than the 14-capture corpus above: **290 ms** is the longest MODE ENTER press ever recorded, over 160 presses, and the 1200 ms fan hold clears it by 4.1×; **330 ms** is the longest ordinary `btn_indicator_cancel` press, over 775 of 779, and the 1000 ms waypoint hold clears it by 3.0×. The full per-button table, the four presses excluded from that 775 and why, and the method's failure mode are in `docs/handlebar-gestures.md`.
