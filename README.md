...
| **Cooling fan** (custom loop, `FAN_ENABLED=1`) | `fan_duty_pct` (%, what the bridge is being given), `fan_target_pct` (%, what was asked for — the two differ only during a kick-start), `fan_driver_enabled` (1 = both IBT-2 enables HIGH, 0 = standby), plus `fan_auto_mode` (1 = the temperature curve is driving, 0 = the slider is) and the two codes that say why the curve chose what it chose, `fan_auto_reason` and `fan_temp_input`. Every one is what the Pi **commanded or decided**, never what it measured: this fan has no tacho and the current-sense pins are unconnected, so nothing here can tell you the rotor is actually turning. Written on change like everything else, which for a fan held at one duty means nothing for an hour. See [`docs/fan-control.md`](docs/fan-control.md) | Pi hardware PWM0 + `pinctrl` |
...
| **BMS state & faults** | `charge_state` (raw System State bitfield) + decoded `bms_state_*` (discharge / charge / balancing / trickle / idle / charge complete / maintenance), `bms_error_flags`, `bms_warning_flags` (raw words) + booleans for the ones worth acting on: cell over/under voltage, over temperature, leak detected, leak detection failed, contactor faults, low SOC, balancing required. Plus `bms_leak_detect_inhibit` — Energica's own `V_IMD_DISABLE`, the vehicle **commanding** the insulation monitor off for the duration of a DC session, since it cannot run against a station-driven bus. It is 1 while a DC session exists in 99.933 % of 156 931 frames and 0 whenever one does not in 100.000 % of 810 934 | CAN `0x201`/`0x605` b7 |
| **Isolation** | `iso_test_1`, `iso_test_2`, `iso_test_total` (10-bit ADC, 512 = ideal), `bms_io_state`, `lmu_comm_warnings` | CAN `0x207`/`0x206` |
| **Charge** | `charge_state`, `dc_v`, `dc_a`, `mains_v`, `mains_a`, `charge_limit_a`, `dc_charge_limit_selected_a`, `charger_enabled`, `charger_max_dc_v`, `charger_max_dc_a`, `bms_post_processor_1` (purpose unknown, logged raw so a ride can name it), `fast_dc_contactor` — the DC fast-charge contactor monitor (Energica's `V_FASTDC_MON_SW`, and the analog wire `A020_FCHG_MON`). It is 1 only on DC: across 1.1 M frames of `0x102` it was set in exactly one interval, a 17-minute DC session that took the pack 30 → 42 %, and 0 through every AC charge including a 48-minute one at 14 A. It rises ~190 ms before `charger_enabled` and ~470 ms before the first amp, which is what a contactor monitor should do. **From the charge manager** (decoded 2026-08-19 across 29 charge sessions, 18 AC and 11 DC — that ECU is only awake with a cable plugged in, and the standing assumption that this meant DC only was wrong; reconciled against Energica's factory DBC on 2026-08-20, see `docs/charge-manager.md`): `fast_dc_target_a` — the DC current the vehicle **asks the station for**, and the only DC-side current on this bus, since a DC session sends no `0x305`/`0x306` at all and `charge_limit_a` reads 0 through one; `fast_dc_target_v`, the DC target voltage that goes with it, which sits a median 13.4 V above the pack; `fast_dc_limit_a` and `ac_supply_limit_a`, the ceilings the **station** offers for each path, each 0 while the other path is live; `fast_dc_limit_max_a` and `fast_dc_limit_max_v`, the vehicle's own configured 75 A and 371 V; `dc_charging` / `ac_charging`, which say current is _flowing_; `charge_type` (1 = AC, 2 = DC) and `charge_manager_state`, which say a _session exists_ — `0x610` b7 reads `0x23` on DC in 99.974 % of 151 096 frames and `0x02` on AC in 99.999 % of 784 599, the cleanest AC/DC discriminator on the bus; plus `charge_manager_status`, `charge_manager_soc` (the SOC the VCU reports outward to the station, which is free to disagree with the BMS's) and `charge_manager_error_src` / `charge_manager_error_code` — the charge manager's own fault code, which `src/can/vcu-flags.ts` had recorded as not broadcast anywhere and which has fired three times in the archive | CAN `0x201`/`0x305`/`0x306`/`0x10a`/`0x300`/`0x102` b3/`0x121`/`0x605`/`0x610`/`0x615`/`0x620`/`0x625` |
...
| **Energy, the cluster's range** | `range_can_km` — CAN `0x412` b2-b3 at 2 Hz, the instrument cluster's own range estimate, computed on the cluster and the only range that survives the Bluetooth link being down. Same bound as `range_km` so the two compare directly. See `docs/can-0x412.md` | CAN `0x412` |
...
| **Drive** | `throttle_pct`, `speed_kmh`, `motor_rpm`, `motor_load_pct`, `dist_since_clear_km`, `motor_torque_nm`, `motor_power_kw` (over Bluetooth; the same records are on CAN `0x410` sub-type 3 and are logged separately as `motor_torque_can_nm` / `motor_power_can_kw`), `odometer_km`, `trip_km`, `vehicle_state`, `vehicle_substate` | CAN `0x109` + OBD-II `0D`/`0C`/`04`/`31` + hub |
| **Drive, the cluster's own numbers** | `dash_speed_kmh`, `motor_torque_can_nm`, `motor_power_can_kw` — CAN `0x410` sub-type 3, the records the instrument cluster transmits and the hub carries over Bluetooth. Torque and power are the Bluetooth pair off the bus instead, logged separately so the transports stay comparable. 🟡 `dash_speed_kmh` is **not** `speed_can_kmh`: the same geared driveline speed through a ~40.26 divisor against 42.0, a median ratio of 1.0433 over 42 705 paired samples, which puts it ~8 % above GPS where a legal speedometer sits. Whether it is literally the number on the dash is **unconfirmed** — one glance at the speedo against the phone settles it. See `docs/can-0x410.md` | CAN `0x410` |
| **Drive, second opinion** | `speed_can_kmh`, `motor_rpm_can`, `odometer_can_km`, `odometer_pulse`, `rolling_backwards` — deliberately **not** merged into the keys above, because comparing the two over a ride is the point and one key with two writers just flaps. Plus the inverter's own limits `current_max_out_a`, `current_max_regen_a`. 🚨 `current_other_a` is **gone**: 0x109 b6-7 is not a current at all — it is Energica's ride-map-and-events bitfield, and the .xdbc reading produced 5069 A. It is replaced by `tc_event` and `eabs_event`, the VCU's own traction-control and eABS interventions, ✅ confirmed by behaviour (`tc_event` fires at 77 % throttle and +138 Nm median, and separates torque even within one throttle band) | CAN `0x104`/`0x109` |
...
| **VCU error & status flags** | `vcu_flags_low`, `vcu_flags_high` — the VCU's own 64-bit flag word, logged raw in two halves so no flag is ever lost, plus booleans for the ones worth an alert: `vcu_err_charge_manager`, `vcu_err_check_modules`, `vcu_check_modules_status`, `vcu_warn_soc_misaligned`, `vcu_12v_power_good`, `vcu_err_system_fault`, `vcu_err_system_fatal_fault`, `vcu_err_system_blocking_fault`, `vcu_err_battery_ot`, `vcu_err_motor_ot`, `vcu_err_drive_ot`, `vcu_err_leak_detect`. **`vcu_err_charge_manager` is the useful one**: it is broadcast, so a charge-manager fault becomes visible passively. ✅ It **has** fired — set in **3 563 frames across four events**, rising 0.162 s after `0x610` publishes an error code in three of them, and held 269.7 s with no code at all in the fourth. ⚠️ Two claims here were wrong until 2026-08-21: that the charge manager's error telemetry "is not broadcast at all and needs a diagnostic session" (`0x610` broadcasts it), and that this bit reads "0 in all 105 736 captured frames — never seen to fire", which was a statement about the sample. Seven of the other booleans genuinely have not been seen to fire. Four bits have been watched moving, one of them landing on the exact second a DC session was unplugged. See `src/can/vcu-flags.ts` | CAN `0x100` |
...
| **Controls & vehicle state** | `high_beam_lamp`, `low_beam_lamp`, `horn` (which the All page latches like a button — a horn blast is as brief as a press), `energized`, `go_request`, `go`, `key_on`, `stand_up` (sidestand retracted), `ignition_button`, `throttle_on`, `moving`, `cruise_active` (cruise control armed — inferred, not in any vendor table, from two events where it followed the cruise ON/OFF button by half a second). ⚠️ The two beam lamps were logged as `charging` and `charge_port_unlocked` until 2026-08-16, on a rider-made `.xdbc`'s word: **both names were simply wrong.** Each agrees with its beam switch in `0x102` b0 across all 1 103 000 frames of the capture corpus with zero disagreements, and the old `charging` bit reads 0 through every real charge. Old rows are correct readings under a wrong name, so the Grafana State panel unions the old key into each lane and the history stays continuous. ⚠️ `brake` (front **or** rear) was logged in this row from June until 2026-08-30 and is **gone**: it was computed from `front_brake` \| `rear_brake`, which have been logged separately since 2026-08-19, and this log stores the bits that were measured rather than combinations a reader can recompute. The rows already written are still correct readings, so the State panel's Brake lane reads them up to the day the halves start and the OR of the halves after it | CAN `0x102` |
| **Rider controls** | Everything the rider's hands and feet move, in one group so the dashboard can show them together. The **buttons**: `btn_mode_left`, `btn_mode_right`, `btn_mode_enter`, `btn_indicator_cancel` (left pod, as VCU discretes), `btn_set_back`, `btn_cruise_enable`, `btn_cruise_set`, `btn_heated_grip` (dashboard inputs) — momentary and short, the median press measured 140 ms and the shortest 30 ms. The **held states**, added 2026-08-19: `high_beam` (b0 bit 6, the switch, which is also how you change dashboard screens), `blinker_left`, `blinker_right` (the lamp outputs, which flash rather than stay on), `front_brake` and `rear_brake` (b2 `0x20`/`0x40`, two keys because they are two circuits — captured frames carry both at once, and the rear pedal alone leaves `front_brake_pressure_bar` at 0). The dashboard latches all of them for 600 ms, counts presses, and describes a hold by its duration instead once it passes a second; see [The dashboard](#the-dashboard). Since 2026-08-30 that treatment is chosen per KEY rather than per group (`public/lib/latched.js`), so `horn` and `ignition_button` — momentary, and in `controls` because that is where their history is — get it without being moved between groups, which `rides.db` would never forget. **Confidence varies per bit and is written down next to each one in `src/can/vcu-digitals.ts` (the `0x102` b0 bits) and `src/can/decode.ts` (the `0x400` ones)**: indicator-cancel and the two cruise buttons are confirmed by what they correlate with, the MODE pair is confirmed as menu buttons **and, since 2026-08-19, as left-vs-right** (by instructed press: eight presses of each button in turn, every block fenced by a counted number of indicator-cancel clicks so the capture labels its own blocks), `btn_set_back` was first seen set that same day, and `btn_heated_grip` has still never been seen set at all — which is expected, since this bike has no heated grips. The b0 indicator **switches** are deliberately not decoded — vcu-digitals.ts records that b0 bit 3 is right and bit 4 is left, measured over 14 650 573 frames, for whenever something needs them | CAN `0x102` b0/b2 + `0x400` b2 |
...
Runs the self-checks that replay **committed** fixtures: the trouble-code transfer above, and `scripts/check-vcu-params.ts` (parameter table, request encoding, framing, the live reads, interpretation, the snapshot diff, and the KWP transport against a simulated micro). `scripts/check-button-decode.ts` replays 25 real `0x102`/`0x400` frames, plus 2 constructed ones that prove only that the decoder is self-consistent, — including the two the cruise buttons were ever recorded on, the one the fast-charge contactor closed in, the frame the right indicator lamp lit on, and the rear-brake-only frame that closed an open question in `src/can/abs.ts` — and also guards the three ways this feature could be switched off without anything else failing: `0x400` dropping out of the kernel RX filters, a button key missing from the registry, and a deadband on a 0/1 signal, which stops it logging after the first sample and does so silently. It additionally walks all 256 values byte 2 can take and asserts that the two brake bits stay on 0x20 and 0x40 respectively, because a mask edited by one drifts on a single value nobody thought to capture. `scripts/check-all-view-tiles.ts` covers which tile each signal gets on the All page, and `scripts/check-brake-lane.ts` runs the ride-summary dashboard's own Brake query against a built database, since that lane is now an OR of two log-on-change series carried forward — a shape whose obvious spelling latches to yes and never comes back down. `scripts/check-freeze-frame.ts` cross-checks Energica's 155 per-fault field shortlists against `src/diagnostics/dtc-table.ts` — a real check across two independently sourced tables, which is why it knows about the two water-pump codes they disagree on — and then replays 31 freeze-frame transfers through the reassembler and decoder — 29 of them the replies the bike actually sent, plus 2 constructed (`P0A07` and `P0514`) whose invented values exercise decode paths the captures don't reach — a zero-current open circuit, a negative int16 and int8, the ×0.1 scaling and the `(X/2)−40` air temperature. Those 2 prove only that the decoder is self-consistent; the 29 captured ones prove the wire format. The shapes the bike never sent — a refusal, a wrong-component answer, a short consecutive frame mid-transfer — are checked separately, frame by frame, rather than replayed as transfers. It also runs `scripts/generate-grafana-dtc.ts --check`, which compares the fault-code table Grafana carries inline against `src/diagnostics/dtc-table.ts` — a copy that once went stale for months without anything on screen looking wrong. CI runs the same command on every PR, so a change that breaks a decoder, the parameter table or that dashboard goes red rather than green.
...
| `WIFI_HOTSPOT_SSID` | **none** | ⚠️ **No default, on purpose** — this is a public repo and an SSID is a personal network identifier. Set it to the name of your phone's hotspot; it is what `wifi_hotspot_seen` and `wifi_network` are measured against. Leave it unset and the poller still records the radio's own state, says so loudly at startup, and never writes a fault dump. |
...
| `SERVICE_WRITE_ENABLED` | **off** | `1` lets the dashboard **change** things on the bike: any of 269 calibration parameters, the service point, the bike's clock, and the stored trouble codes. ⚠️ One of the two opt-**in** switches here, `FAN_ENABLED` being the other — everything else defaults on and takes a `0` to disable — so a Pi nobody has told about it cannot write to a motorcycle's EEPROM. It is the only one of the two that can reach the bike at all. Separate from `SERVICE_MODE_ENABLED` on purpose. See [Changing something on the bike](#changing-something-on-the-bike). |
...
#   31 freeze-frame transfers (29 captured, 2 constructed) through the real
#   reassembler and decoder
```

> ✅ **The wire format is verified.** The _request_ was always proven twice over — the factory tool sent `0x17` to A8 29 times in a 2026-08-08 capture and got 29 positive replies, and Energica's own code builds exactly that frame. This box used to add that the _response layout_ "has never been captured" and that the record-count byte was "genuinely open". Both were wrong: the 29 replies were on `7E0` all along, and the search that missed them looked at `7C0`, which carries only requests. They are committed in `scripts/captured-freeze-frames.ts`, `recordCount` is 1 on all 29 and asserted, and the header is 5 bytes — settled by decoding all 29 against physical bounds, since the lengths alone cannot separate a 5-byte header with a trailer from a 6-byte header without one. One byte per reply is still undecoded. See [§5.1 and §11.3.1](docs/diagnostics-and-checks.md).
...
Every reply is printed as **raw hex first**, before any decode. That is the point of the run: the BULK LOG reply layout (`0x35`/`0x36`/`0x37`) is still unverified — `0x17` and `0x18` are now both captured and confirmed, so the bytes are the result and the decode is a hypothesis printed beside them.

> ⚠️ **For the bulk log, the request side is proven and the reply side is not.** (`0x17` and `0x18` are settled — see above and §11.3.1. This box is about `0x35`/`0x36`/`0x37`.) The check asserts that this repo's segmenter reproduces `A8 10 0C 35 12 FF FF FF` — a frame captured off this bike — byte for byte, and that the `0x18` request matches the one recovered from the manufacturer's code. This box used to say that **no multi-frame reply and no flow-control frame had ever been captured on this channel**, in either direction. That is no longer true, and was already false when written: the 2026-08-08 capture holds 1229 First Frames and 43 786 Consecutive Frames on `7E0`, plus 1229 flow-control frames on `7C0`. Twenty-eight of those replies are committed in `scripts/captured-freeze-frames.ts` and replayed through the production reassembler. The one reply replayed with real bytes behind it is A8's bank-2 identifier `0x2001`, reconstructed from two independent live records that had to agree and did. Everything else is constructed from the documented framing.
>
...
**Stationary and charging is allowed**, deliberately — you cannot test a DC charge-current limit on a bike that is not plugged in. Three things can witness the charge: the DC fast-charge contactor (`0x102` b3 bit 0), the onboard AC charger's frames (`0x305`/`0x306`, which do not exist on DC), and a settled `charge_manager_state` (`0x610` b7, which covers both) — and any of them being fresh lets `energized` be set, because a charging bike's HV side is up by definition. ⚠️ The charge manager reporting an EMPTY INLET cancels all three: a failed attempt on 2026-08-09 broadcast the settled AC value for 14.8 s with nothing in the inlet. `speed_can_kmh`/`motor_rpm_can` may also be absent while charging, though the reason given for that rule is refuted (#194). Nothing else relaxes — the `0x102` bits must still be live and clear, and a fresh speed reading that says the wheel is turning still refuses.

### Probing one identifier

The sweep covers the 277 parameters the bike's own table describes, plus the 25 more A8's own firmware table serves that `params.ecf` does not name — 302 identifiers, bank 1, on the two VCU micros. Those 25 (the odometer master, the last-service stamp and 18 with nothing traced) are shown unnamed, with raw bytes and no typed value, because their width comes from a firmware image and their signedness from nowhere at all: [`docs/vcu-parameters.md`](docs/vcu-parameters.md) §2. The **`/vcu-probe` endpoint** reaches anything else — name the micro, the bank and the index and it reads that one: `curl -X POST -H 'X-Cool-Eva: service-mode' '<pi>/vcu-probe?target=A9&bank=2&index=258'`. It had a panel on the service sheet until 2026-08; that was removed as clutter for something done a handful of times ever, and what it was for is recorded on issue #51. What lives out there and nothing here could reach before: **bank 2 is live data** rather than stored settings. Same header, same gate and same single-flight as a sweep.

> ⚠️ This briefly offered a **charge manager** target, on CAN `0x7C3`/`0x7E3`. That pair is wrong and has been removed: **`0x7E3` is the dashboard's request id**, so the option could have questioned the dashboard while the page said otherwise, and `RequestFrameIDs.CHM = 0x7C3` turns out to be a dead enum the manufacturer's own code references nowhere. Node `0xA4` is the charge manager's real 11-bit identity, but the ECU actually answers on **29-bit ISO-TP** — request `0x18DA09F1`, response `0x18DAF109` — which needs `ext: true`, its own RX filter and its own addressing math. That is a feature rather than a constant, so the target is gone rather than re-pointed; the full note is in `docs/vcu-parameters.md`, "The charge manager was a target here, and it was WRONG". Whoever adds it should know it is **off-bus when parked** (it answers only during a live charging session), that identification reads need no SecurityAccess, and that its deeper access uses a CRC-16/CCITT algorithm that is **not** the VCU's bit-swap.
...
Writing is **off unless you ask for it**: `SERVICE_WRITE_ENABLED=1`. It is one of the two switches here that default to off — `FAN_ENABLED` is the other, and it drives an accessory off the Pi's own GPIO rather than anything on the bike — and it is separate from `SERVICE_MODE_ENABLED` on purpose: reads and writes are not the same risk and must not share an off button.

Every parameter in the bike's table may be written — **269** of the 277, the other eight being four names that appear twice and so have no unambiguous address. ⚠️ This was a closed allowlist of five until 2026-08-21; the argument for it was that an index two away from the five is `CELL_OVERVOLTAGE`, and that argument was about INDEX typos on an interface that only ever took a name.

What the five below still have is **research** — measured bounds and a warning written by someone who went and looked. The other 264 carry the datatype's full range and say, on screen, that nothing here knows what they do. Two guards do the work absence used to: every write is a compare-and-swap against a fresh read, and `writeTargetProblemIn` refuses any parameter the bike's own table puts at a different index (26 of the 28 carried tables disagree about at least one of the 269). The five, with their researched ranges:
...
| `unwritable` | the table is carried, and one of the five RESEARCHED parameters is not called that on it (the other 264 are checked per write instead, by `writeTargetProblemIn`) | `src/vcu/write-targets.ts` needs an entry correct for that table. (No shipped table does this — all 28 agree on ids 16, 48, 49, 258 and 259) |
...
- **Read the last-service stamp** — A8 bank 1, ids 1000-1003, a 32-bit count of seconds since 2000-01-01 UTC plus a 32-bit odometer. Read-only. **First read 2026-09-08: all four answered zero**, so the block is there and no service point has ever been set on this bike ([`docs/service-stamp.md`](docs/service-stamp.md)). They sit outside `params.ecf`'s 1…277, so no name table describes them — a sweep does now read them, since #219 — and they are deliberately **not** logged as signals — reading them costs a KWP session, and they move about once a year.
