...
| **Controls & vehicle state** | `high_beam` (which is also how you change dashboard screens), `brake`, `blinker_left`, `blinker_right`, `horn`, `energized`, `go_request`, `go`, `key_on`, `stand_up` (sidestand retracted), `ignition_button`, `throttle_on`, `moving`, `charging`, `charge_port_unlocked`, `cruise_active` (cruise control armed — inferred, not in any vendor table, from two events where it followed the cruise ON/OFF button by half a second) | CAN `0x102` |
