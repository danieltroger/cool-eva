...
| **Charge** | `charge_state`, `dc_v`, `dc_a`, `mains_v`, `mains_a`, `charge_limit_a`, `dc_charge_limit_selected_a`, `charger_enabled`, `charger_max_dc_v`, `charger_max_dc_a`, `bms_post_processor_1` (purpose unknown, logged raw so a ride can name it), `fast_dc_contactor` — the DC fast-charge contactor monitor (Energica's `V_FASTDC_MON_SW`, and the analog wire `A020_FCHG_MON`). It is 1 only on DC: across 1.1 M frames of `0x102` it was set in exactly one interval, a 17-minute DC session that took the pack 30 → 42 %, and 0 through every AC charge including a 48-minute one at 14 A. It rises ~190 ms before `charger_enabled` and ~470 ms before the first amp, which is what a contactor monitor should do. **`dc_charge_limit_selected_a` is the DC current limit _you_ picked on the bike's charging screen** — as opposed to the ceiling that screen offers it up to (75 A here, VCU parameter 258 `MAX_DC_CHG_CURRENT`, service-tool-only). Those are two different numbers, and conflating them is what made "why do I only get 63 A when it says 75?" unanswerable for months. Dial it **down** and the current obeys exactly — over 15 plateaus and 53 268 measured-current samples the delivered current never once exceeded it, and a commanded 5/10/20/35 A was met to the amp within a second. Dial it **up** and it may not be honoured, because the station's envelope or the VCU's pack-temperature derate binds first. ⚠️ **An event, not a stream**: `0x121` fires when the dial moves and never otherwise (5 of 10 captured DC sessions contain no such frame at all, because it was never touched), nothing rebroadcasts it, so the tile shows the last setting seen and greys out afterwards like `waypoint_seq`. The AC side of the same dial is `charge_limit_a`, which _is_ continuous. See `src/can/charge-setpoint.ts` | CAN `0x201`/`0x305`/`0x306`/`0x10a`/`0x300`/`0x102` b3/`0x121` |
