...
Telemetry for a **watercooled 2021 Energica Eva Ribelle**. A Raspberry Pi inside
...
- **2× MAX31865 + PT100 probes** over SPI — coolant **in** and **out** of the custom
  battery watercooling loop.
- **[8devices Korlan USB2CAN](https://shop.8devices.com/usb2can/korlan/)** plugged into
  the bike's OBD port → `can0` (in-kernel `usb_8dev`, no driver install). 500 kbit, 11-bit.
...
Everything is logged **on change** (so steady values don't spam the DB) into a small
SQLite database. `🟡` = decoded but the scaling/layout is not yet verified on a ride.
...
| **Energy** | `inst_consumption_wh`, `residual_energy_wh` (available energy) 🟡 | CAN `0x025`/`0x204` |
...
- `src/can/` — `socket` (can0 bring-up + raw channel), `decode` (broadcast frame
  decoders), `obd` (OBD-II poll loop), `signals`/`registry` (log-on-change core).
...
The app is a `systemd` service on the Pi (Node 24, TypeScript run directly via
`--experimental-strip-types`), serving `http://<pi>/` on port 80.
...
Dashboard provisioned from `grafana/dashboards/cooling.json` (battery temp vs
coolant, ΔT across the pack, charge, cells, drive, …).

## Notes

- The CAN bus is **read-only**: passive broadcast decode + standard OBD-II *read*
  requests only. No KWP/UDS writes.
- Coolant history predating the CAN integration is preserved (migrated into the
  current schema; the original table is kept as a backup).
