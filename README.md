...
| **Cooling fan** (custom loop, `FAN_ENABLED=1`) | `fan_duty_pct` (%), `fan_driver_enabled` (1 = both IBT-2 enables HIGH, 0 = standby). Both are what the Pi **commanded**, never what it measured: this fan has no tacho and the current-sense pins are unconnected, so nothing here can tell you the rotor is actually turning. Written on change like everything else, which for a fan held at one duty means nothing for an hour. See [`docs/fan-control.md`](docs/fan-control.md) | Pi hardware PWM0 + `pinctrl` |
