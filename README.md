...
The dashboard menu's **Update** button does exactly that pull and restart from the phone, showing git's output. It pulls as root, which is why the Pi's `origin` is https and not ssh — [INSTALL.md §3](INSTALL.md) explains why root cannot use `pi`'s key.
