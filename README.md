...
| **ABS & brakes** | `wheel_speed_front_kmh`, `wheel_speed_rear_kmh` — the ABS module's own two wheels, the only front-wheel measurement on the bike. `front_brake_pressure_bar` — **brake line pressure**, which nothing else here carries; it is exactly 0 in every frame the front brake switch is open and reached 17 during a garage lap. `abs_warning_lamp`. ⚠️ Both scalings are Energica's stated ones, not measured: the wheel-speed constant does not reproduce this bike's dash speed (front reads ~4 % high, rear ~4 % low, and the two disagree with each other by ~9 %), so treat them as km/h to within a few percent and not as a speedometer. The pressure's "bar" has nothing on the bus to check it against at all. See `src/can/abs.ts` | CAN `0x0A0` |
...
| **All** | Every signal the bike is producing, grouped and filterable. The **buttons** group renders differently from the rest: the handlebar buttons are momentary, and the median press is 140 ms — one or two frames of a 60 Hz display — so a raw 1/0 readout would flicker and be gone. Each button tile latches lit for 600 ms after the bit drops and keeps a press count with the time of the last one. Trust the count over the light: a count that climbs while the tile never visibly lights is a working button whose flash the browser dropped, and a tile stuck lit with a count of 1 is a fault, not a press. Since 2026-08-19 the group also holds the indicators, the high beam and the two brakes, which are **not** momentary, so a tile that has been down for over a second says how long instead ("held 4 s") — a clock reading rather than a list of which keys are held states, because the populations overlap: the high beam is a 0.27 s flash-to-pass 163 times out of 180 and a held state the other 17. The blinkers are the one hand-named exception. They are lamp outputs and a running indicator toggles them at **1.46 Hz**, so counting rising edges would report 1881 presses where the rider signalled 323 turns; a falling edge on those two is not believed until the bit has been at 0 for 700 ms, which is an empty valley in the measured gap distribution. That rule is deliberately not applied to `high_beam`, whose three-flashes-in-2 s tab gesture it would erase |
