...
| `unread` | one or both micros have never been asked | **one read.** The refusal names it: parameter 277 `TABLE_TYPE_uS` on the A8, `22 11 15`, read-only, no SecurityAccess, expect `0x4017` |

> ⚠️ **This bike is in the `unread` state today (2026-08-16), so the gate is shut.** The A9's copy (276) was read on 2026-06-14 and says 16407; the A8's copy (277) has never been read by anyone — and id 249, the one id where the two candidate tables disagree, is an A8 parameter. The two micros hold separate EEPROMs, so one answering is not confirmation of the other. "Probe one identifier" in service mode can do this read, and it is one frame.
