...
- **Read the last-service stamp** — A8 bank 1, ids 1000-1003, a 32-bit count of seconds since 2000-01-01 UTC plus a 32-bit odometer. Read-only. **First read 2026-09-08: all four answered zero**, so the block is there and no service point has ever been set on this bike ([`docs/service-stamp.md`](docs/service-stamp.md)). They sit outside `params.ecf`'s 1…277, so no sweep reaches them, and they are deliberately **not** logged as signals — reading them costs a KWP session, and they move about once a year.
