...
**To break a line across a transport dropout you need `insertNulls`, and `spanNulls` has to get out of its way. They are not independent settings.** `insertNulls: <ms>` ("Disconnect values" in the UI) inserts a null once a series goes that long without a sample. `spanNulls` then decides whether that null survives the frame join. Measured by mutation on `ride-summary.json`'s **Inverter torque** panel, Grafana 11.3:

| `insertNulls` | `spanNulls` | result                                                                  |
| ------------- | ----------- | ----------------------------------------------------------------------- |
| `false`       | `60000`     | line drawn straight through a 5.3 h dropout — no nulls exist to span    |
| `20000`       | `true`      | **flat line is back** — `true` deletes the nulls `insertNulls` inserted |
| `20000`       | `false`     | ✅ line breaks at the dropout, normal sampling still continuous         |

So `spanNulls: true` **silently cancels** `insertNulls`, and any numeric `spanNulls` above `insertNulls` bridges the gap just the same. Set `insertNulls` to the threshold and `spanNulls` to `false`. The first attempt here set only `spanNulls: 60000` and changed nothing on screen, which is the trap that leads to the wrong conclusion — with `insertNulls: false` there are no nulls in the frame to span, so `spanNulls` alone looks inert and is not.

⚠️ **Several `spanNulls: true` settings in these dashboards are therefore doing the opposite of what their descriptions claim.** "Motor power and torque" credits `spanNulls` for drawing "exactly what was recorded" while in fact holding `motor_torque_nm` flat across BLE dropouts of hours. It matters wherever two independently-transported signals share one axis: a held line next to a live one reads as the two sources disagreeing, which is exactly the "looks like a finding" failure this file warns about elsewhere.

**Pick the threshold off a histogram of that signal's own gaps, not off a percentile.** ⚠️ A percentile will mislead you here. `motor_torque_nm`'s p99 is 1.31 s, which makes 60 s look enormously safe; it is not, because the distribution is not one population with a tail. Over the whole log: 113 358 gaps under 5 s, then only **79** across the entire 5-35 s span, then a distinct **556** sitting at 35-45 s — a recurring hub stall, with the bike moving throughout. A 60 s threshold bridges that whole second population and hides **332 000** inverter rows behind held line segments, against 510 at any threshold from 10 s to 30 s. The valley between the two populations is what you want, and only a histogram shows it. Where there is no valley — the module temperatures below, where a healthy sensor goes 48 minutes between samples — no threshold works and the answer is a dedicated liveness signal instead.
...
⚠️ **`charge-manager.json` and `route-map.json` carry that guard throughout, and so do the three queries behind `ride-summary.json`'s "Inverter torque — commanded vs delivered"; no other panel does.** They are safe only for as long as their default `to: now` range is left alone, because `now` is before 2060 and the misdated rows sort after it. Change one to an absolute range ending past 2033 and two things break at once: every axis stretches to 2060 and squashes the real data into a sliver at the left, and — worse, because it looks like an answer — the held-value tiles that deliberately drop their lower bound (`r.ts <= $__to`, §Querying log-on-change data above) return a 2060 row. Measured on the current `rides.db`: `odometer_can_km` held to a `$__to` in 2060 reads **17 497.1 km stamped 2060-08-08** instead of the true latest **17 866.4 km from 2026-08-09**. `pack_a` has 1 740 such rows, `dc_v` 492, `pack_v` 301, `mains_a` 254. Retrofitting the guard to the seven dashboards that still lack it entirely is a mechanical change nobody has made yet, and it is not free of judgement — a few panels exist to inspect data health and might legitimately want to see the corruption.
