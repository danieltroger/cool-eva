...
const REPLAY: ReplayCase[] = [
  ...
  // makes these regression tests rather than a copy of the decoder's own arithmetic.
  {
    ...
    why: "2026-08-04 20:05:20, mid-plateau of the 2026-08-04 DC session. b0-1 are one 16-bit target: 0x013A = 314 V, and 0x200 read 300.5 V at that instant — the +13.5 V the request sits above the pack, which is what the retired 242.5 offset was measuring. b2 = 63 A requested against 63.2 delivered, b3 = 40 % against 40",
    expect: { fast_dc_target_v: 314, fast_dc_target_a: 63, charge_manager_soc: 40 },
  },
  {
    id: 0x615,
    frame: "2D 01 49 11 00 00 00 00",
    why: "2026-08-08 17:49:24 — the highest DC current in the whole corpus, 73 A, at a low 17 % SOC. 0x200 read 288.0 V / 73.0 A / 17 % against a 0x012D = 301 V target. This is the case that would break if b2 were ever read as anything but plain amps",
    expect: { fast_dc_target_v: 301, fast_dc_target_a: 73, charge_manager_soc: 17 },
  },
  {
    id: 0x615,
    frame: "5D 01 07 63 00 00 00 00",
    why: "2026-08-09 18:15:22 — the far end of a DC taper, 99 % SOC and only 7 A left. 0x200 read 335.5 V / 6.7 A / 99 % against a 0x015D = 349 V target, so the constant +13.5 V holds at the top of the pack's range as well as the middle. b1 is still 0x01: the 16-bit target never leaves 256…511 V",
    expect: { fast_dc_target_v: 349, fast_dc_target_a: 7, charge_manager_soc: 99 },
  },
  {
    id: 0x615,
    frame: "2C 01 00 1F 00 00 00 00",
    why: "2026-08-03 22:31:22, five hours into the 6.8 h overnight AC charge at 1 A. b2 = 0 while 1.0 A of AC charge current flows — the property that makes fast_dc_target_a DC-specific rather than a general charge current. Note the VCU still sends a 0x012C = 300 V target on AC, which is why this frame is not a DC-only one",
    expect: { fast_dc_target_v: 300, fast_dc_target_a: 0, charge_manager_soc: 31 },
  },
  {
    id: 0x610,
    frame: "5E 00 00 00 F1 05 01 23",
    why: "the DC state, at the same instant as the 63 A frame above. 0x5E is the factory fields all at once — CM_CHGMODE_REQ = 2 (DC), relay requested, inlet present and locked, WAIT_AUTH = 2 — and b7 = 0x23. The error pair is 0, as it is in 968 129 of 968 629 frames",
    expect: {
      charge_manager_status: 0x5e,
      charge_manager_state: 0x23,
      charge_manager_error_src: 0,
      charge_manager_error_code: 0,
    },
  },
  {
    id: 0x610,
    frame: "19 00 00 00 F1 05 01 02",
    why: "the AC state, from the overnight session. 0x19 is CM_CHGMODE_REQ = 1 (AC) + inlet present + locked, no relay and WAIT_AUTH = 0, and b7 = 0x02. Neither b7 value ever appears in the other mode: 99.974 % of 151 096 DC-current frames read 0x23 and 99.999 % of 784 599 AC-mains frames read 0x02",
    expect: {
      charge_manager_status: 0x19,
      charge_manager_state: 0x02,
      charge_manager_error_src: 0,
      charge_manager_error_code: 0,
    },
  },
  ...
  },
  {
    ...
    why: "DC, same instant as the 63 A frame. b2 = 0x4B = 75 is MAX_DC_CHG_CURRENT read back from the VCU; b0-1 are the 16-bit V_CM_V_LIMIT at 0x016B = 363 V; b4 = 0x12 has bit 5 clear (DC flowing) and bit 2 clear (no AC)",
    expect: { fast_dc_limit_max_v: 363, fast_dc_limit_max_a: 75, dc_charging: 1, ac_charging: 0 },
  },
  {
    id: 0x625,
    frame: "6B 01 4B FF 2C 00 00 00",
    why: "AC overnight: b4 = 0x2C, bit 2 set and bit 5 set. Same b2 = 75 and same 363 V — both are static across DC, AC and parked alike, which is why they are configuration constants and not negotiated values",
    expect: { fast_dc_limit_max_v: 363, fast_dc_limit_max_a: 75, dc_charging: 0, ac_charging: 1 },
  },
  {
    id: 0x625,
    ...
    expect: { fast_dc_limit_max_v: 363, fast_dc_limit_max_a: 75, dc_charging: 0, ac_charging: 0 },
  },
  {
    id: 0x625,
    frame: "00 00 00 00 00 00 00 00",
    why: "⚠️ SYNTHETIC, and the reason 0x625 checks its own invariants. b4's DC bit is read INVERTED, so an all-zero payload has bit 5 clear and would decode to dc_charging = 1 — a false charge claim that bounds.js cannot reject, because 1 is a legitimate value for a flag. b1 = 0x01 and b3 = 0xFF in 100.000 % of 1 571 614 real frames, so requiring them turns this shape back into no reading",
    expect: {},
    absent: ["fast_dc_limit_max_v", "fast_dc_limit_max_a", "dc_charging", "ac_charging"],
  },
  {
    id: 0x625,
    frame: "FF FF FF FF FF FF FF FF",
    why: "⚠️ SYNTHETIC — the mirror image, and the other shape a dead or disconnected sender produces. Here bit 2 is set, so without the invariant check it would decode to ac_charging = 1. b1 = 0xFF fails the gate — which under the 16-bit reading is the same as saying V_CM_V_LIMIT is outside 256…511 V",
    expect: {},
    absent: ["fast_dc_limit_max_v", "fast_dc_limit_max_a", "dc_charging", "ac_charging"],
  },
  {
    ...
    why: "⚠️ SYNTHETIC — b0 = 0 makes the 16-bit target exactly 256 V, a round number inside bounds.js's band that would look like a request rather than a fault. ⚠️ The guard's ORIGINAL reason is gone: it was written when b0 was read as a voltage with a 242.5 V offset, where b0 = 0 decoded to that offset. b0 spans 28…94 over all 941 765 captured frames and is never 0, so this should be unreachable either way. The other two fields still decode",
    expect: { fast_dc_target_a: 0, charge_manager_soc: 31 },
    absent: ["fast_dc_target_v"],
  },
  {
    id: 0x620,
    frame: "00 00 00 00 00 00 00 00",
  ...
  },
  {
    id: 0x615,
    frame: "FF FF FF FF FF FF FF FF",
    why: "⚠️ SYNTHETIC — the all-ones dead-sender shape, and the worst-exposed frame in this group. Ungated it decodes to a 0xFFFF = 65 535 V target and fast_dc_target_a = 255 A; bounds.js passed both until the entries added alongside this case (its V band is [-50, 900] and the A fallback [-1000, 1000]), and only charge_manager_soc = 255 was ever caught, by the % band. 255 A on the only DC current on this bus is a number that reaches a chart's autoscale and then a conclusion. b1 = 0x01 with b4-7 = 00 in 100.000 % of 941 765 real frames is what turns it back into no reading",
    expect: {},
    absent: ["fast_dc_target_v", "fast_dc_target_a", "charge_manager_soc"],
  },
  {
    id: 0x615,
    frame: "00 00 00 00 00 00 00 00",
    why: "⚠️ SYNTHETIC — the other dead-sender shape. The b0 = 0 guard alone would already drop the voltage, but not fast_dc_target_a = 0 and charge_manager_soc = 0, and those two are worse than a silly number: they read as a healthy 'plugged in, nothing requested, empty pack'. b1 = 0x00 fails the invariant, which under the 16-bit reading is the same as saying the target is below 256 V",
    expect: {},
    absent: ["fast_dc_target_v", "fast_dc_target_a", "charge_manager_soc"],
  },
  {
    id: 0x610,
    frame: "00 00 00 00 00 00 00 00",
    why: "⚠️ SYNTHETIC — all-zero, and the frame with the least defence of the group: the state bytes are logged raw and bounded to [0, 255] in bounds.js ON PURPOSE, so that a state nobody has seen yet is not rejected — which leaves bounds.js unable to reject anything for them and this invariant the only check there is. b4 is 0xF1 in 100.000 % of 968 629 real frames, and 0x00 is not it",
    expect: {},
    absent: ["charge_manager_status", "charge_manager_state", "charge_manager_error_src", "charge_manager_error_code"],
  },
  {
    id: 0x610,
    frame: "FF FF FF FF FF FF FF FF",
    why: "⚠️ SYNTHETIC — all-ones, which ungated decodes to charge_manager_status = 255 and charge_manager_state = 255, both inside the deliberate [0, 255] band, plus a −1 error code. Note what is NOT gated: b1-3, the error pair, which is non-zero in three real episodes. Gating on them would have thrown away the most interesting DC data in the archive",
    expect: {},
    absent: ["charge_manager_status", "charge_manager_state", "charge_manager_error_src", "charge_manager_error_code"],
  },
  {
    ...
    why: "DC, same instant as the 63 A frame: b2 = 2 (DC) and b7 = 1, `V_IMD_DISABLE` — the VCU commanding the isolation monitor off for the duration, since it cannot run against a station-driven bus. 1 while a DC session exists in 99.933 % of 156 931 frames. Note b0 = 0x11 puts V_CHGMODE_FDB (bits 3-4) at 2 as well, the echo b2 is the wider copy of",
    expect: { charge_type: 2, bms_leak_detect_inhibit: 1 },
  },
  {
    id: 0x605,
    frame: "0F 00 01 01 00 00 00 00",
    why: "AC overnight: b2 = 1 (AC) and leak detection left running. b0 = 0x0F puts V_CHGMODE_FDB at 1, agreeing with b2 as it does in 99.822 % of 967 865 frames. b3 duplicates b2 in every frame of the corpus, which is why only b2 is emitted",
    expect: { charge_type: 1, bms_leak_detect_inhibit: 0 },
  },
  // Added 2026-08-20 with the factory-DBC reconciliation. See docs/charge-manager.md.
  {
    id: 0x610,
    ...
    expect: {
      ...
      charge_manager_state: 0x23,
      charge_manager_error_src: 7,
      charge_manager_error_code: 853,
    },
  },
  {
    id: 0x610,
    frame: "2A 08 4D 04 F1 05 01 23",
    why: "2026-08-08 17:41:32 — the SECOND fault episode, 44 minutes before that day's DC session and missed entirely by the 2026-08-19 pass, which recorded the error bytes as non-zero 'in exactly one place in the whole corpus'. A different source (8) with a different code (1101) is what makes the pair two fields rather than one 24-bit number",
    expect: {
      charge_manager_status: 0x2a,
      charge_manager_state: 0x23,
      charge_manager_error_src: 8,
      charge_manager_error_code: 1101,
    },
  },
  {
    id: 0x610,
    frame: "5E 00 00 00 F1 04 60 23",
    why: "⚠️ SYNTHETIC — the DC frame above with ANOTHER BIKE's b4-6. This byte triple is TMS_Version: ours reads F1 05 01 and the Ribelle this decode was cross-checked against reads F1 04 60. The gate was all three bytes until 2026-08-20, which decoded this ECU on exactly one motorcycle; it is b4 alone now, and this case is what stops it being tightened back",
    expect: { charge_manager_status: 0x5e, charge_manager_state: 0x23 },
  },
  {
    id: 0x625,
    frame: "73 01 4B FF 32 00 00 00",
    why: "2026-08-09 17:55:22 — V_CM_V_LIMIT after it moved, 0x0173 = 371 V against the 363 V every earlier frame carries. b0 was on the undecoded list as 'a one-way change that no charge event lines up with'; as a 16-bit voltage the change lands 91 s INTO a DC session and flickers between the two values for 15.5 s first. b2 is unchanged at 75, which is what says the two limits are separate parameters",
    expect: { fast_dc_limit_max_v: 371, fast_dc_limit_max_a: 75, dc_charging: 0, ac_charging: 0 },
  },
  {
    id: 0x125,
