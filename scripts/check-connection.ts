...
import { HEARTBEAT_MS, MAX_CLIENT_BACKLOG_BYTES, broadcastTo } from "../src/ws.ts";
...

{
  const it = newWorld();
  it.link.start();
  check("a socket is opened at startup", it.sockets.length === 1);
  check("nothing is called live before a message arrives", it.world.status === "connecting");

  it.newest().handlers.opened(it.newest());
  check("the handshake alone is still not live — the snapshot has not arrived", it.world.status === "connecting");

  it.newest().handlers.received(it.newest());
  check("the first message is what makes it live", it.world.status === "live");

  it.hide();
  check("hiding the page closes the socket", it.sockets[0].closed);
  check("...and the header stops claiming a link", it.world.status === "offline");
  check("...and nothing new is opened in its place", it.sockets.length === 1);
  ...
  it.advance(5 * 60_000);
  check("no socket is opened during five minutes hidden", it.sockets.length === 1);
  check("the header still says nothing is arriving", it.world.status === "offline");

  it.show();
  check("unlocking opens exactly one new socket", it.sockets.length === 2 && it.open().length === 1);
  check("...immediately, with no reconnect delay in front of it", it.world.status === "connecting");
  it.newest().handlers.received(it.newest());
  check("...and it is live as soon as the Pi's snapshot lands", it.world.status === "live");
}
...

{
  ...
  check("...so it cannot relabel the header live", it.world.status === "offline");

  it.show();
  const second = it.newest();
  ...
  check("...and only that one makes it live", it.world.status === "live");
}
...

{
  ...
  for (let cycle = 0; cycle < 4; cycle += 1) {
    it.hide();
    check(`cycle ${cycle}: nothing is left open while hidden`, it.open().length === 0);
    it.show();
    opened.push(it.sockets.length);
    check(`cycle ${cycle}: exactly one socket is open`, it.open().length === 1);
  }

  check("four wakes cost four sockets, not more", it.sockets.length === 5);
  check(
  ...
  const stale = it.sockets[1];
  const current = it.newest();
  stale.handlers.closed(stale);
  check("a late close from an abandoned socket opens nothing", it.sockets.length === 5);
  check("...and does not disturb the live one", it.open().length === 1 && !current.closed);

  stale.handlers.opened(stale);
  check("a late handshake on an abandoned socket is closed rather than adopted", stale.closed);
  check("...and the current socket is still the current socket", !current.closed && it.open().length === 1);
  ...
  it.advance(3 * RECONNECT_DELAY_MS);
  check("no retry was ever queued for either of them", it.sockets.length === 5);
}
...

{
  ...
  it.advance(SILENCE_LIMIT_MS - POLL_MS);
  check(
    `nothing happens at ${(SILENCE_LIMIT_MS - POLL_MS) / 1000} s — a parked bike must not churn sockets`,
    !first.closed && it.world.status === "live" && it.sockets.length === 1
  );

  it.advance(2 * POLL_MS);
  check(`past ${SILENCE_LIMIT_MS / 1000} s of silence the socket is dropped`, first.closed);
  check("...and the header says so", it.world.status === "offline");
  check("...without a close event ever being delivered", it.sockets.length === 1);

  it.advance(RECONNECT_DELAY_MS - POLL_MS);
  check("the retry waits out the backoff", it.sockets.length === 1);
  it.advance(2 * POLL_MS);
  check("...and then opens exactly one socket", it.sockets.length === 2 && it.open().length === 1);
  ...
  it.advance(RECONNECT_DELAY_MS + POLL_MS);
  check("a third attempt follows", it.sockets.length === 3);
  const before = it.sockets.length;
  it.advance(SILENCE_LIMIT_MS - POLL_MS);
  check("...and one attempt is one socket, not a spin", it.sockets.length === before);
}
...

{
  ...
  check("a close is reported as offline at once", it.world.status === "offline");
  check("...and does not reconnect on the spot", it.sockets.length === 1);

  it.advance(RECONNECT_DELAY_MS - POLL_MS);
  check(`nothing before ${RECONNECT_DELAY_MS / 1000} s`, it.sockets.length === 1);
  it.advance(2 * POLL_MS);
  check("...one attempt after it", it.sockets.length === 2);
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const socket = it.newest();
    socket.handlers.closed(socket);
    check(`attempt ${attempt}: nothing is left open`, it.open().length === 0);
    it.advance(RECONNECT_DELAY_MS + POLL_MS);
    check(`attempt ${attempt}: exactly one new socket`, it.open().length === 1);
  }
}

{
  ...
  it.hide();
  first.handlers.closed(first);
  it.advance(10 * RECONNECT_DELAY_MS);
  check("a close while hidden schedules nothing", it.sockets.length === 1);
  it.show();
  check("becoming visible is what reconnects", it.sockets.length === 2);
}

{
  ...
  const it = newWorld();
  it.world.hidden = true;
  it.link.start();
  check("start() on a hidden page opens nothing", it.sockets.length === 0);
  check("...and says offline rather than connecting for ever", it.world.status === "offline");
  it.advance(10 * SILENCE_LIMIT_MS);
  check("...and still nothing, however long it waits", it.sockets.length === 0);
  it.show();
  check("...until it is looked at", it.sockets.length === 1);
}
...

{
  ...
  for (const def of SIGNALS) {
    signals[def.key] = { value: -123.45, unit: def.unit, group: def.group, ts };
  }
