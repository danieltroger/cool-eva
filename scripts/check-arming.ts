...
// injected, since the two `performance.now()` defaults are the lines that actually ship.
//
...
const VIEW_PATHS = ["public/views/vcu-write.js", "public/views/charge-current.js", "public/views/charge-stop.js"];
const VIEW_SOURCES = new Map<string, string>();
for (const path of VIEW_PATHS) {
  VIEW_SOURCES.set(path, await readFile(new URL(`../${path}`, import.meta.url), "utf8"));
}
...
/** The keys vcu-write.js compares `armed.val` against by name, read off the comparisons themselves. */
const literalKeys = [
  ...new Set(
    [...sourceOf("public/views/vcu-write.js").matchAll(/armed\.val [!=]== "([^"]+)"/g)].map(match => match[1])
  ),
];
...
const SITES = VIEW_PATHS.flatMap(path => firingSites(path, sourceOf(path)));
console.log(`   found: ${SITES.map(site => shortName(site)).join(", ")}`);
check("exactly five controls on the dashboard act on a second tap", SITES.length === 5);

for (const site of SITES) {
  const where = shortName(site);
  const keyTest = site.body.indexOf("armed.val !== ");
  const dwellTest = site.body.indexOf("armDwellElapsed()");
  ...
  const armingBranch = site.body.slice(keyTest, dwellTest);
  check(
...
for (const path of ARMING_CONSUMERS) {
  ...
  for (const match of source.matchAll(/armed\.val\s*=(?!=)\s*([^;\n]*)/g)) {
...
for (const path of ARMING_CONSUMERS) {
  const source = await readFile(new URL(`../${path}`, import.meta.url), "utf8");
  const named = importedFromArming(source);
...
/** The view's source, which §5 onwards read more than once. */
function sourceOf(path: string): string {
  const source = VIEW_SOURCES.get(path);
  if (source === undefined) {
    throw new Error(`check-arming: ${path} was never read — VIEW_PATHS and the sections below disagree`);
  }
  return source;
}
...
interface FiringSite {
  ...
  name: string;
  /** The `onclick` body, or "" when it could not be found — which fails every assertion on it. */
  body: string;
  /** The button's props ahead of the handler, where `onkeydown` lives. */
  props: string;
...
/**
 * Every control in one view that ACTS on a second tap, found from the action rather than from
 * the gate: a site that skipped the gate entirely would be invisible to a search for the gate.
 */
function firingSites(file: string, source: string): FiringSite[] {
  return [...source.matchAll(/\bvoid (perform[A-Za-z]*)\(/g)].map(match => {
    ...
    const buttonAt = source.lastIndexOf("button(", handlerAt);
    const found = block !== null && block.end > callIndex;
    return {
      file,
      name: match[1],
      body: found ? source.slice(block.start, block.end + 1) : "",
      props: buttonAt === -1 || handlerAt === -1 ? "" : source.slice(buttonAt, handlerAt),
      callAt: found ? callIndex - block.start : -1,
    };
  });
}
