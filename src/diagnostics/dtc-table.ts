...
//
// SECOND SOURCE, 2026-08-15 — Energica's own EMsuite service tool. EMsuite ships
// the same table as JSON embedded in EMSuite.exe, one {id, code, symptom, title}
// record per fault and 153 of them; see obd-garage/EMSUITE_FILES.md and the
// extract at `<Energica_Manuals>/EMsuite/files/em_fault_codes.csv`. That is an
// independent path to the same data — a shipped binary rather than a PDF read by
// eye — so where the two agree the transcription below is corroborated, not
// merely careful. They share 147 (component, symptom) pairs and 144 of those
// carry an IDENTICAL OBD code. All three disagreements are argued at the entries
// themselves: (44,0) and (44,2), where EMsuite corrected a swap, and (61,2),
// where it replaced a dual code with a single one.
//
...
export const DTC_TABLE: DtcTableEntry[] = [
  ...
  // KEPT 2026-08-15 despite being the one entry EMsuite has no record of. Its
  // absence there is a gap in EMsuite, not evidence against the code: the two
  // sources disagree about coverage in both directions (EMsuite carries six the
  // PDF omits), so neither one's silence disproves the other's listing.
  entry(
    4,
    5,
    "U0115",
  ...
  // for symptom 0 and P0A05 "…CIRCUIT/OPEN" for symptom 2; EMsuite's embedded
  // table has them swapped, and three things say EMsuite is right. The old rows
  ...
  // ADDED 2026-08-15 from EMsuite; the type-approval PDF has no rows for these.
  // Components 51, 52 and 60 are the battery's three statistics records, and
  // their existence is why this file no longer calls those component numbers
  ...
  // note for it. EMsuite gives one ordinary code instead, C0065, and that wins:
  // it was the only "two codes at once" entry in either source, and a scan tool
  // has one code slot per fault to receive it in, so the dual form could never
  // have been transmitted as written. The name below is EMsuite's title, since
  // the PDF's name described the pair rather than C0065.
  entry(
    61,
    2,
    "C0065",
