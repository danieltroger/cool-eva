// `VCU/params.ecf` — Energica's own parameter-table text, verbatim — and the strict
...
export interface VcuParameter {
  ...
  micro: VcuMicro;
  /**
   * The `[SECTION]` heading it sat under in `params.ecf`, e.g. "EVSE". Grouping only.
   *
   * ⚠️ Null when the table being described renames this id, because the heading is
   * then a claim about a parameter that is not there any more. Energica's own bundles
   * carry no sections at all (PARAM_TABLES.md §1), so this is `params.ecf`'s
   * contribution and it only travels as far as the name does.
   */
  section: string | null;
  /**
   * The value the `params.ecf` bike carries. NOT this bike's, NOT a default, and
   ...
   *
   * ⚠️ Null under the same rule as `section`: a renamed id's old value belongs to the
   * old parameter.
   */
...
export function parseParameterFile(text: string): VcuParameter[] {
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (line.startsWith("[")) {
      if (!line.endsWith("]")) {
        throw new Error(`param-file: line ${lineNumber} looks like a section but does not close: ${line}`);
      }
...
function parseParameterLine(line: string, section: string, lineNumber: number): VcuParameter {
  ...
  if (columns.length !== 6) {
    throw new Error(`param-file: line ${lineNumber} has ${columns.length} columns, expected 6: ${line}`);
  }
  ...
  if (micro !== "A8" && micro !== "A9") {
    throw new Error(`param-file: line ${lineNumber} names an unknown micro ${micro}`);
  }
  const otherBikeValue = Number(valueText);
  if (!Number.isFinite(otherBikeValue)) {
    throw new Error(`param-file: line ${lineNumber} has a non-numeric value ${valueText}`);
  }
  if (section.length === 0) {
    throw new Error(`param-file: line ${lineNumber} sits before any [SECTION] heading`);
  }
  return {
    index,
 ...
 */
export function PARAMETER_FILE_TEXT(): string {
  return `
[VSM]
